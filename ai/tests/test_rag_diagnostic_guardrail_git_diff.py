...
def test_v5_6_official_metric_scored_execution_fail_closed_does_not_mutate_protected_or_export_training_surfaces():
    import ai.scripts.rag_eval as runner

    report = runner.check_run("v5_6")
    generated_text = json.dumps(report, ensure_ascii=False)
    ...
    assert report["backend_preflight"]["status"] == "BACKEND_UNAVAILABLE_FAIL_CLOSED"
    assert report["backend_preflight"]["available"] is False
    assert report["backend_preflight"]["noop_backend_used"] is False
...
def test_v5_6_2_official_metric_backend_enabled_preflight_does_not_mutate_protected_or_export_training_surfaces():
    import ai.scripts.rag_eval as runner

    report = runner.check_run("v5_6_2")
    generated_text = json.dumps(report, ensure_ascii=False)
    result = report["official_metric_scored_result"]
