...
LOGICAL_RUN_KEY = "v5_6_3"
SHORT_RUN_ID = "v5_6_3_official_metric_backend_probe_and_scored_execution_nonprod"
...
V5_6_2_PREFLIGHT_LOGICAL_RUN_KEY = v562.LOGICAL_RUN_KEY
V5_6_2_PREFLIGHT_RUN_ID = v562.SHORT_RUN_ID
V5_6_2_PREFLIGHT_REPORT_JSON = v562.SHORT_REPORT_PATH
...
FAILURE_CATEGORIES = set(PREFLIGHT_FAILURE_CATEGORIES)
PASS_CATEGORY = "pass"
ENABLE_ENV_VAR = "RAG_V5_6_3_ENABLE_OFFICIAL_METRIC_EXECUTION"
BACKEND_ENV_VAR = "RAG_V5_6_3_LOCAL_LLM_BACKEND"
BASE_URL_ENV_VAR = "RAG_V5_6_3_LOCAL_LLM_BASE_URL"
MODEL_ENV_VAR = "RAG_V5_6_3_LOCAL_LLM_MODEL"
SCORER_BACKEND_ENV_VAR = "RAG_V5_6_3_SCORER_BACKEND"
SCORER_BASE_URL_ENV_VAR = "RAG_V5_6_3_SCORER_BASE_URL"
SCORER_MODEL_ENV_VAR = "RAG_V5_6_3_SCORER_MODEL"
...
def _load_v562_preflight_report(root: Path, source_report: Mapping[str, Any]) -> dict[str, Any]:
    try:
        report = registry.load_report(V5_6_2_PREFLIGHT_LOGICAL_RUN_KEY, root=root)
    except registry.ReportResolutionError:
        report = v562.build_report(root=root, source_report=source_report, execute=False)
...
def _load_metric_input_rows(root: Path) -> list[dict[str, Any]]:
    ...
    if len(rows) != EXPECTED_ROW_COUNT:
        raise ValueError("v5_6_3 source official metric input row count must be exactly 29")
    return rows
...
def _source_artifact_validation(
    ...
    preflight_report_path = root / V5_6_2_PREFLIGHT_REPORT_JSON
    ...
    preflight_validation = preflight_report.get("source_artifact_validation") or {}
    preflight_hashes = preflight_report.get("artifact_sha256") or {}
    preflight_input_hash = _clean(
        preflight_validation.get("source_official_metric_input_sha256")
        or preflight_hashes.get("source_official_metric_input_jsonl_sha256")
    )
    preflight_input_path = _clean(
        preflight_validation.get("source_official_metric_input_path")
        or preflight_report.get("artifact_paths", {}).get("source_official_metric_input_jsonl")
    )
    return {
        ...
        "v5_6_2_report_path": V5_6_2_PREFLIGHT_REPORT_JSON.as_posix(),
        "v5_6_2_report_artifact_status": "present" if preflight_report_path.exists() else "materialized_in_memory",
        "v5_6_2_report_sha256": sha256_file(preflight_report_path) if preflight_report_path.exists() else "",
        "v5_6_2_report_official_metric_input_path": preflight_input_path,
        "v5_6_2_report_source_official_metric_input_sha256": preflight_input_hash,
        "path_matches_v5_6_2_report": preflight_input_path == SOURCE_OFFICIAL_METRIC_INPUT_PATH.as_posix(),
        "sha256_matches_v5_6_2_report": preflight_input_hash == actual_input_hash,
        "row_count": len(rows),
        ...
        "row_count_matches_v5_6_2_report": preflight_report.get("official_metric_input_rows") == len(rows),
        "protected_namespace_policy": "read_only_v5_5_input_v5_6_baseline_and_v5_6_2_preflight_reports_only",
    }
...
def _answer_probe_prompt() -> str:
    return json.dumps(
        {
            "task": "v5_6_3_non_gold_answer_generation_probe",
...
def _scorer_probe_prompt() -> str:
    return json.dumps(
        {
            "task": "v5_6_3_non_gold_scorer_contract_probe",
    ...
    *,
    ...
    try:
        ...
        if not _schema_valid_answer(answer_probe):
            return _preflight_fail(
                base,
                category="answer_generation_probe_failed",
        ...
        return _preflight_fail(
            base,
            category="answer_generation_probe_failed",
            status="ANSWER_GENERATION_PROBE_FAILED_FAIL_CLOSED",
    ...
    try:
        if scorer_client is not None:
            ...
            if not ok:
                scorer_category = (
                    "scorer_contract_unavailable"
                    if category == "answer_generation_model_unavailable"
                    else "scorer_backend_unreachable"
                )
                return _preflight_fail(
                    base,
                    category=scorer_category,
                    status=f"{scorer_category.upper()}_FAIL_CLOSED",
                    blockers=blockers,
                )
        ...
        return _preflight_fail(
            base,
            category="scorer_contract_probe_failed",
            status="SCORER_CONTRACT_PROBE_FAILED_FAIL_CLOSED",
            blockers=[f"SCORER_CONTRACT_PROBE_FAILED:{type(exc).__name__}:{exc}"],
        )
    ...
    return {
        **base,
        "status": "BACKEND_PROBE_PASSED_NONPROD",
        "answer_generation_available": True,
...
def _answer_prompt(row: Mapping[str, Any]) -> str:
    return json.dumps(
        {
            "task": "v5_6_3_official_metric_answer_generation_nonprod",
            "instructions": [
...
def _scorer_prompt(row: Mapping[str, Any], generated: Mapping[str, Any]) -> str:
    return json.dumps(
        {
            "task": "v5_6_3_official_metric_answer_citation_scoring_nonprod",
            "instructions": [
                "Return exactly one JSON object.",
...
def _fail_closed_result(
    ...
    *,
    ...
    return {
        ...
        "quality_metric_blocked_reason": category,
...
def _score_rows(
    ...
    *,
    ...
    if not backend_preflight.get("available"):
        category = _clean(backend_preflight.get("failure_category")) or "unknown_fail_closed"
        reason = "; ".join(str(item) for item in backend_preflight.get("blockers") or [backend_preflight.get("status")])
        return _fail_closed_result(rows, category=category, reason=reason)
    ...
    for row in rows:
        ...
        try:
            ...
            if category == "scoring_runtime_failed":
                raise RuntimeError("scorer reported scoring_runtime_failed")
            ...
            detail = f"SCORING_RUNTIME_FAILED:{type(exc).__name__}:{exc}"
            return _fail_closed_result(rows, category="scoring_runtime_failed", reason=detail)
        result_row = {
            "query_id": row["query_id"],
            "track": row["track"],
            "source_v5_4_review_row_id": row["source_v5_4_review_row_id"],
...
def build_report(
    *,
    ...
    prior_preflight = _load_v562_preflight_report(repo_root, source)
    rows = _load_metric_input_rows(repo_root)
    row_count_by_track = dict(Counter(str(row.get("track") or "") for row in rows))
    validation = _source_artifact_validation(repo_root, source, baseline, prior_preflight, rows)
    ...
    report = {
        ...
        "artifact_paths": {
            ...
            "v5_6_2_preflight_report_json": V5_6_2_PREFLIGHT_REPORT_JSON.as_posix(),
        ...
        "v5_6_2_preflight_run_id": V5_6_2_PREFLIGHT_RUN_ID,
        "v5_6_2_preflight_status": prior_preflight.get("status"),
        ...
        "basis_for_ambiguous_engineering_choices": (
            "v5_6_1 is already occupied by a product-preview runtime bridge and v5_6_2 is an immutable "
            "backend-enabled preflight lane in the live tree, so v5_6_3 is the next repo-consistent official metric "
            "execution key; the runner reuses only v5_5 official_metric_input.jsonl, checks the v5_5/v5_6/v5_6_2 "
            "source hash contract, keeps v5_6 and v5_6_2 artifacts immutable, and fails closed before any real "
            "scoring unless the v5_6_3 execution gate plus answer/scorer probes pass"
        ),
        **duplicate_summary,
    }
...
def write_report_bundle(root: Path | str, report: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    preflight_report_path = repo_root / V5_6_2_PREFLIGHT_REPORT_JSON
    ...
    if preflight_report_path.exists():
        child_hashes["v5_6_2_preflight_report_json_sha256"] = sha256_file(preflight_report_path)
    elif _clean(validation.get("v5_6_2_report_sha256")):
        child_hashes["v5_6_2_preflight_report_json_sha256"] = _clean(validation.get("v5_6_2_report_sha256"))
...
def status_event(report: Mapping[str, Any], *, artifact_hashes: Mapping[str, str]) -> dict[str, Any]:
    ...
    return {
        ...
        "event_type": "v5_6_3_official_metric_backend_probe_and_scored_execution_nonprod",
...
def _replace_current_status_block(progress_text: str, report: Mapping[str, Any]) -> str:
    ...
    replacement = (
        ...
        "backend probe/scored-execution lane over the exact v5_5 official metric input rows. `current` resolves "
        "to `v5_6` so the v5_6 artifacts remain immutable fail-closed baseline, while `v5_6_3`, `v5_6_2`, "
        "`v5_5`, `v5_4`, `v5_3`, `v5_2`, `v5_1`, `v5_0`, and `v4_7_18` remain directly checkable.\n\n"
        "Current run board:\n"
        f"- source_of_truth: `{SOURCE_OFFICIAL_METRIC_INPUT_PATH.as_posix()}` from `{SOURCE_RUN_ID}`; "
        "user-approved gold packet ingestion from the v5_4 user-owned approval packet; source hash matches the "
        "v5_5, v5_6, and v5_6_2 recorded source hashes.\n"
        ...
        "- immutable_v5_6_2_preflight: backend-enabled preflight lane; execution_gate_disabled; "
        "scored_answer_rows=0; answer_quality_metric_computed=false; failure_category_counts=`{'execution_gate_disabled': 29}`.\n"
        ...
        f"- Immutable v5_6_2 scored result: `{v562.OFFICIAL_METRIC_SCORED_RESULT_PATH.as_posix()}`.\n"
        f"- Immutable v5_6_2 failure attribution: `{v562.FAILURE_ATTRIBUTION_PATH.as_posix()}`.\n"
        f"- Current explicit v5_6_3 report: `{SHORT_REPORT_PATH.as_posix()}`.\n"
        f"- Current explicit v5_6_3 backend preflight: `{BACKEND_PREFLIGHT_RESULT_PATH.as_posix()}`.\n"
        f"- Current explicit v5_6_3 scored result: `{OFFICIAL_METRIC_SCORED_RESULT_PATH.as_posix()}`.\n"
...
def update_docs(root: Path | str, report: Mapping[str, Any]) -> None:
    ...
    progress_block = (
        ...
        f"`{SOURCE_OFFICIAL_METRIC_INPUT_PATH.as_posix()}` and performs backend probe before any official metric "
        ...
        "v5_6 and v5_6_2 artifacts remain immutable. No raw prompt/response payloads are written; duplicate "
    ...
    if result["answer_quality_metric_computed"] is True:
        ...
        measurements_block = f"""## v5_6_3 official metric backend probe scored execution
...
def _assert_no_raw_payload_keys(value: Any) -> None:
    common.assert_no_raw_payload_keys(value, set(RAW_PAYLOAD_FORBIDDEN_KEYS), context="v5_6_3")
...
def _require_artifact_paths(report: Mapping[str, Any]) -> None:
    ...
    if report.get("artifact_paths") != expected:
        raise ValueError("v5_6_3 artifact path drift")


def _require_exact_source_rows(report: Mapping[str, Any]) -> None:
    if "official_metric_input_rows_payload" in report:
        raise ValueError("v5_6_3 protected official input payload embedded")
    ...
    if report.get("official_metric_input_row_ref_count") != EXPECTED_ROW_COUNT or len(refs) != EXPECTED_ROW_COUNT:
        raise ValueError("v5_6_3 official metric input row ref count drift")
    ...
    for index, row in enumerate(refs):
        if row.get("row_index") != index:
            raise ValueError("v5_6_3 official metric input row ref order drift")
        if not _clean(row.get("row_sha256")) or not _clean(row.get("citation_locator_sha256")):
            raise ValueError("v5_6_3 official metric input row ref hash drift")
        for forbidden in (
            "expected_answer_ko",
...
def _require_written_artifacts(report: Mapping[str, Any], *, root: Path | str) -> None:
    ...
    for path_key, payload_key, hash_key, kind in (
        ...
        if not rel_path:
            raise ValueError(f"v5_6_3 artifact path missing: {path_key}")
        path = repo_root / str(rel_path)
        if not path.exists():
            raise ValueError(f"v5_6_3 artifact missing: {path_key}")
        actual = json.loads(path.read_text(encoding="utf-8")) if kind == "json" else read_jsonl(path)
        if actual != report.get(payload_key):
            raise ValueError(f"v5_6_3 artifact payload drift: {path_key}")
        if hashes.get(hash_key) != sha256_file(path):
            raise ValueError(f"v5_6_3 artifact hash drift: {hash_key}")
...
def check_report(report: Mapping[str, Any], *, root: Path | str | None = None) -> None:
    ...
    if report.get("run_id") != SHORT_RUN_ID or report.get("short_run_id") != SHORT_RUN_ID:
        raise ValueError("v5_6_3 run_id mismatch")
    if report.get("canonical_long_run_id") != CANONICAL_LONG_RUN_ID:
        raise ValueError("v5_6_3 canonical_long_run_id mismatch")
    if report.get("status") not in {STATUS, SCORED_STATUS}:
        raise ValueError("v5_6_3 status mismatch")
    if report.get("logical_run_key") != LOGICAL_RUN_KEY:
        raise ValueError("v5_6_3 logical run key mismatch")
    if report.get("source_run_id") != SOURCE_RUN_ID or report.get("source_logical_run_key") != SOURCE_LOGICAL_RUN_KEY:
        raise ValueError("v5_6_3 source run mismatch")
    if report.get("current_resolves_to") != CURRENT_RESOLVES_TO:
        raise ValueError("v5_6_3 current resolution mismatch")
    if report.get("v5_6_baseline_run_id") != V5_6_BASELINE_RUN_ID:
        raise ValueError("v5_6_3 v5_6 baseline mismatch")
    if report.get("v5_6_2_preflight_run_id") != V5_6_2_PREFLIGHT_RUN_ID:
        raise ValueError("v5_6_3 v5_6_2 preflight mismatch")
    if report.get("non_production") is not True:
        raise ValueError("v5_6_3 must remain non-production")
    ...
    if report.get("approval_scope") != expected_scope:
        raise ValueError("v5_6_3 approval scope drift")
    ...
    if report.get("official_metric_input_rows_consumed") != EXPECTED_ROW_COUNT:
        raise ValueError("v5_6_3 official metric input rows consumed drift")
    if report.get("row_count_by_track") != EXPECTED_ROWS_BY_TRACK:
        raise ValueError("v5_6_3 row count by track drift")
    validation = report.get("source_artifact_validation") or {}
    if validation.get("path_matches_v5_5_report") is not True:
        raise ValueError("v5_6_3 source artifact path mismatch")
    if validation.get("sha256_matches_v5_5_report") is not True:
        raise ValueError("v5_6_3 source artifact sha256 mismatch")
    if validation.get("path_matches_v5_6_report") is not True:
        raise ValueError("v5_6_3 v5_6 source artifact path mismatch")
    if validation.get("sha256_matches_v5_6_report") is not True:
        raise ValueError("v5_6_3 v5_6 source artifact sha256 mismatch")
    if validation.get("path_matches_v5_6_2_report") is not True:
        raise ValueError("v5_6_3 v5_6_2 source artifact path mismatch")
    if validation.get("sha256_matches_v5_6_2_report") is not True:
        raise ValueError("v5_6_3 v5_6_2 source artifact sha256 mismatch")
    if validation.get("row_count") != EXPECTED_ROW_COUNT or validation.get("row_count_matches_v5_5_report") is not True:
        raise ValueError("v5_6_3 source artifact row count drift")
    if validation.get("row_count_matches_v5_6_report") is not True:
        raise ValueError("v5_6_3 v5_6 source artifact row count drift")
    if validation.get("row_count_matches_v5_6_2_report") is not True:
        raise ValueError("v5_6_3 v5_6_2 source artifact row count drift")
    if report.get("duplicate_supporting_evidence_policy") != (
        "recorded_for_locator_precision_audit; row-level citation_locator remains authoritative"
    ):
        raise ValueError("v5_6_3 duplicate evidence policy drift")
    if report.get("duplicate_supporting_evidence_id_count") != 1:
        raise ValueError("v5_6_3 duplicate evidence summary drift")
    if report.get("protected_namespaces_touched") != []:
        raise ValueError("v5_6_3 protected namespace touched")
    for key in CLOSED_FALSE_KEYS:
        if report.get(key) is not False:
            raise ValueError(f"v5_6_3 closed surface opened: {key}")
