...
def write_markdown(path: Path, payload: Mapping[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Existing OCR fallback enabled: `{parse.get('existing_ocr_fallback_enabled')}`",
        f"- Existing OCR fallback attempted/succeeded: `{parse.get('existing_ocr_fallback_attempted_count')}` / `{parse.get('existing_ocr_fallback_success_count')}`",
        f"- OCR-used pages/blocks: `{parse.get('ocr_used_page_count')}` / `{parse.get('ocr_used_block_count')}`",
        f"- OCR confidence avg: `{parse.get('ocr_confidence_avg')}`",
        "",
        "## Synthetic Anchors",
        "",
        ...
        f"- Lower-trust OCR evidence objects: `{evidence.get('lower_trust_ocr_object_count')}`",
        ...
        "- OCR fallback evidence remains lower-trust diagnostic context, not native-text-equivalent gold evidence.",
        "",
        "## Next Steps",
        "",
