...
def main(argv: list[str] | None = None) -> int:
    ...
    write_json(Path(summary["artifact_paths"]["failure_attribution_json"]), failure_attribution)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    if args.run_id not in {RUN_ID, V2_RUN_ID, V2_1_RUN_ID}:
        raise SystemExit(
            f"unsupported run id {args.run_id!r}; expected {RUN_ID!r}, {V2_RUN_ID!r}, or {V2_1_RUN_ID!r}"
        )
...
def is_source_bound_manifest_run(run_id: str) -> bool:
    return run_id in {V2_RUN_ID, V2_1_RUN_ID}
...
def failure_attribution_for_row(row: Mapping[str, Any], *, summary: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if row.get("infrastructure_blocker_category") in {
