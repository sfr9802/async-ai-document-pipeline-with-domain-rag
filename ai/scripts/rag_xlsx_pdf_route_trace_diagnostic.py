"""Write diagnostic-only XLSX/PDF route trace reports."""
...
from eval.harness.xlsx_pdf_route_trace import (  # noqa: E402
    ...
    DiagnosticReporter,
    TraceConfig,
    default_route_report_paths,
)


def main(argv: list[str] | None = None) -> int:
    args = parse_args(argv)
    default_json, default_md = default_route_report_paths(args.date)
    config = TraceConfig(
        date=args.date,
    ...
    payload = reporter.route_trace_report(config)
