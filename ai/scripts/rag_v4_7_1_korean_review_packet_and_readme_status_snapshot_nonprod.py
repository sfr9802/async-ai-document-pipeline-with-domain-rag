...
def _review_row(row: Mapping[str, Any]) -> dict[str, str]:
    ...
    machine = {
        ...
        "source_preview_redacted": NOT_SUPPLIED,
        "evidence_preview_redacted": NOT_SUPPLIED,
        "locator_preview_redacted": NOT_SUPPLIED,
        "page_or_sheet_locator_redacted": NOT_SUPPLIED,
        "machine_notes": (
            "v4_7 등록 manifest는 후보/질의 ID와 해시 가능한 source-family identity만 제공했습니다. "
            "실제 질의문, 근거 preview, locator context는 이 packet에서 생성하지 않았고 사람 검수 보류로 남깁니다."
        ),
...
def build_review_rows(candidate_manifest_path: Path) -> list[dict[str, str]]:
    rows = read_jsonl(candidate_manifest_path)
    review_rows = [_review_row(row) for row in rows if clean(row.get("source_family")).upper() in {"PDF", "XLSX"}]
...
def _review_summary(review_rows: Sequence[Mapping[str, str]]) -> dict[str, Any]:
...
def build_artifacts(*, candidate_manifest_path: Path, output_dir: Path = OUTPUT_DIR) -> dict[str, Any]:
    v4_7_artifacts = v47.build_artifacts(candidate_manifest_path=candidate_manifest_path)
    v47.check_report(v4_7_artifacts["report"])
    review_rows = build_review_rows(candidate_manifest_path)
    counts = _count_by_family(review_rows)
    summary = _review_summary(review_rows)
    ...
    report = {
        ...
        "remaining_user_owned_actions": [
            "actual Korean query text and evidence/context must be supplied or adjudicated by the user before labels can be final",
...
4. v4_7 등록 manifest에는 실제 질의문과 evidence context가 포함되지 않았으므로, 이 packet의 빈 `질의문`은 자동 생성하지 않습니다.
...
The v4_7_1 Korean human review packet is generated under `ai/eval/reports/rag-ingestion/quality/{RUN_ID}/` with `report.json`, `review_packet_ko.xlsx`, `review_packet_ko.csv`, `review_packet_ko.jsonl`, `review_guidelines_ko.md`, and `review_summary_ko.json`.
...
def update_progress_doc(report: Mapping[str, Any]) -> None:
    entry = (
        ...
        "decision columns as `미검수`, `보류`, or blank; actual query/evidence text was not supplied by the v4_7 "
        "registration manifest and is not invented. It keeps `official_metric=false`, `official_metric_input_rows=0`, "
...
- Run: `{RUN_ID}`
- Primary artifacts: `report.json`, `review_packet_ko.xlsx`, `review_packet_ko.csv`, `review_packet_ko.jsonl`, `review_guidelines_ko.md`, `review_summary_ko.json` under `{repo_relative(OUTPUT_DIR)}`.
- Source evidence: v4_7 pre-official registration report plus external manifest SHA-256 `{EXPECTED_V4_7_MANIFEST_SHA256}`; actual LLM response examples are from v3_22 answer-allowed rows only.
...
def run_write(*, candidate_manifest_path: Path, output_dir: Path = OUTPUT_DIR, update_docs: bool = True) -> dict[str, Any]:
    artifacts = build_artifacts(candidate_manifest_path=candidate_manifest_path, output_dir=output_dir)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        if args.candidate_manifest is not None:
            artifacts = build_artifacts(candidate_manifest_path=args.candidate_manifest, output_dir=output_dir)
            check_report(artifacts["report"])
    ...
    report = run_write(candidate_manifest_path=args.candidate_manifest, output_dir=output_dir, update_docs=True)
