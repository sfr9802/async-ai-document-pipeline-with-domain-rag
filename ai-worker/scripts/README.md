...
`ai-worker/scripts/` is the canonical home for worker-owned smoke, ingestion,
readiness, eval, dataset, and report-generation commands.

Run commands from `ai-worker/` unless a script documents another working
directory:
...
Default script inputs and outputs should stay inside `ai-worker/`:
