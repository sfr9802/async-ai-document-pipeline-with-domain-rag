...
def parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--readiness-report",
        default="ai-worker/eval/reports/rag-ingestion/answer_recovery_embedding_readiness.json",
    )
...
def write_outputs(config: Mapping[str, Any], report: Mapping[str, Any]) -> None:
    ...
    md_path = readiness.resolve_path(
        paths.get(
            ...
            "ai-worker/eval/reports/rag-ingestion/answer_recovery_existing_embedding_retrieval_probe.md",
        )
    )
    csv_path = readiness.resolve_path(
        paths.get(
            "existing_embedding_retrieval_probe_csv",
            "ai-worker/eval/reports/rag-ingestion/answer_recovery_existing_embedding_retrieval_probe.csv",
        )
    )
