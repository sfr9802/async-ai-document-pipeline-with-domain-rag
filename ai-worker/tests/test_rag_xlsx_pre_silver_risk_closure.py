...
MODULE_PATH = ROOT / "ai-worker" / "scripts" / "rag_xlsx_pre_silver_risk_closure.py"
...
def test_xlsx_special_rows_remain_non_official():
    rows = {
        ...
        for row in closure.read_csv_rows(
            ROOT / "ai-worker" / "eval" / "eval_queries" / "gold_queries_xlsx_human_review_normalized_v0.csv"
        )
    }
...
def test_no_silver_rows_in_official_denominator():
    rows = closure.read_csv_rows(
        ROOT / "ai-worker" / "eval" / "eval_queries" / "gold_queries_xlsx_human_review_official_positive_v0.csv"
    )
...
def test_pre_silver_report_counts_match_probe_artifacts():
    report = json.loads(
        (ROOT / "ai-worker" / "eval" / "reports" / "rag-ingestion" / "xlsx_pre_silver_risk_closure_20260507.json")
...
def test_pre_silver_report_verification_blocks_are_consistent():
    report_path = ROOT / "ai-worker" / "eval" / "reports" / "rag-ingestion" / "xlsx_pre_silver_risk_closure_20260507.json"
