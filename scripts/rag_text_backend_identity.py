...
DEFAULT_OUTPUT = Path("reports/rag_text_backend_identity_report.json")
...
EVIDENCE_PATHS = {
    ...
    "vector_tools": Path("ai-worker/app/capabilities/rag_orchestrator/vector_tools.py"),
    "fixture_tools": Path("ai-worker/app/capabilities/rag_orchestrator/tools.py"),
    "worker_config": Path("ai-worker/app/core/config.py"),
}
...
def inspect_static_identity(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "vector_adjacent_identity": {
            ...
            "default_artifact_dir": "../rag-data" if "../rag-data" in worker_config_text else None,
