...
- Scope: non-production source-native candidate-surface rebuild for XLSX/PDF actual-RAG diagnostics only. Checkpoint A now marks the current `SourceAtomNonprodRouteSelectedCandidateSurfaceV1` state dirty for Checkpoint B, so the corrected gold strict-v2 and silver strict-v3 report lane is retained only as historical dirty-run context. `bounded_evidence_gate_v1`, selected-evidence-only citations, `official_metric=false`, `official_metric_input_rows=0`, production routing closed, and protected current/latest/source-registry/gold/silver/qrels/labels/expected/denominator surfaces stayed unchanged.
- Checkpoint A audit note (2026-06-21): no new metric comparison should be made from this partial candidate collection. Checkpoint B must select a fresh run ID/checkpoint or recreate the collection and produce a complete manifest before any gold29, RunStore-only locator, silver500, or docs/reviewer metric lane resumes.
- Candidate index measurement: `reports/rag_eval/weaviate_source_atom_index_manifest_nonprod_route_selected_candidate_surface_v1/index_manifest.json` reports `collection_name=SourceAtomNonprodRouteSelectedCandidateSurfaceV1`, `valid=true`, `indexed_count=136280`, `vectorized_object_count=136184`, `metadata_only_object_count=96`, `failed_count=0`, `checkpoint_resumed=true`, `skipped_count=74624`, `upserted_count_this_run=61656`, `production_namespace=false`, `diagnostic_hash_vector_used=false`, and `faiss_used_for_active_retrieval=false`. Corrected reports record `candidate_surface_rebuild.index_manifest_sha256=sha256:ce82ba9bec39e54902d35812b2b79659a26510058b7cb59733cf0a5c1f544673`, matching the manifest file bytes.
- Corrected report-only artifacts: gold `reports/rag_eval/actual_rag_eval_gold29_xlsx_pdf_candidate_surface_same_source_row_strict_v2_local_bge_m3_local_llm_det_nonprod_20260620/report.json` and silver `reports/rag_eval/actual_rag_eval_silver500_xlsx_candidate_surface_same_source_row_strict_v3_local_bge_m3_det_nonprod_20260620/report.json` each contain only `report.json`. They resolved `ai/eval/configs/weaviate_route_selected_candidate_surface_v1.json`, used locally cached `BAAI/bge-m3` embeddings (`vector_dim=1024`), kept `--output-mode single`, `--retrieval-surface source-native`, `--retrieval-backend weaviate-hybrid`, `--evidence-gate-mode enforce`, `--agentic-planner-mode off`, deterministic selected-evidence composer, and no `--append-registry` or `--write-latest`.
- Baseline artifacts are `reports/rag_eval/actual_rag_eval_gold29_xlsx_pdf_vector_db_det_reindexed_20260615/report.json` and `reports/rag_eval/actual_rag_eval_silver500_xlsx_vector_db_det_reindexed_20260615/report.json`; these are exact comparison artifacts, not `latest` or `current` pointers. Earlier 2026-06-19 candidate v1/planner-axis and 2026-06-20 sanitized v5/source-date-alias attempts are superseded context.

| lane | exact baseline | corrected run | measured delta |
| --- | --- | --- | --- |
| gold strict E2E | `0/29=0.0` | `0/29=0.0` | `0.0` |
| gold exact/alias answer correctness | `0/29=0.0` | `0/29=0.0` | `0.0` |
| gold evidence_recall@10 | `10/29=0.344828` | `10/29=0.344828` | `0.0` |
| gold gate allowed/insufficient | `5/24` | `5/24` | `0/0` |
| gold citation-supported rows | `22` | `18` | `-4` |
| gold provisional judged correctness | baseline context only | `3/29=0.103448` | diagnostic-only |
| gold provisional E2E | baseline context only | `1/29=0.034483` | diagnostic-only |
| gold XLSX/PDF residual value-missing-axis | `23` | `23` | `0` |
| silver strict E2E | `0/0=N/A` | `0/0=N/A` | N/A, 500 strict rows skipped |
| silver gate allowed/insufficient | `7/493` | `8/492` | `+1/-1` |
| silver citation-supported rows | `379` | `303` | `-76` |
| silver judged answer correctness provisional | `4/500=0.008` | `3/500=0.006` | `-0.002` |
| silver weak_evidence_match_recall@10 | baseline context only | `0/500=0.0` | diagnostic-only |
| silver unsupported-after-gate | `0.0` | `0.0` | `0.0` |

- Corrected gold report details: `candidate_surface_rebuild.report_only_diagnostic=true`, `external_archive_profiled=false`, `external_archive_indexed=false`, `latest_current_mutated=false`, `source_registry_mutated=false`, `production_namespace=false`; `residual_anchor_matrix.uses_expected_fields_as_runtime_inputs=false`, `uses_gold_fields_as_runtime_inputs=false`, and `uses_qrels_or_labels_as_runtime_inputs=false`. The heuristic/overfit diagnostics report no row-specific, dataset-specific, or literal case-term configured rules.
- Corrected silver report details: strict metrics are explicitly N/A because `answerability_unknown_not_in_*_denominator=500`; silver remains diagnostic coverage only. Its residual `selected_evidence_has_value_missing_axis=478` and citation-supported regression mean this lane does not support quality, promotion, production, or official metric claims.
- Final RunStore-only locator measurement: `reports/rag_eval/actual_rag_eval_gold29_xlsx_locator_runstore_same_source_row_strict_v3_local_bge_m3_local_llm_20260620/run.sqlite` contains only `run.sqlite` and validates with `validate_xlsx_locator_run_store`. It records `runs.anchor_classifier_model=gemma4-e2b-local`, `official_metric=0`, `official_metric_input_rows=0`, `anchor_classifier_raw_payload_written=0`, `items=29`, `retrieved_contexts=146`, `selected_evidence=32`, `tool_invocations=11`, `tool_candidates=31`, `gate_results=58`, `residuals=58`, and `guardrails=12`.
- RunStore locator result: `candidate_count=31`, `accepted_candidate_count=0`, `complete_validated_axis_candidate_count=0`, `validated_axis_split_across_candidates=1`, `source_row_context_candidate_count=0`, `source_row_context_doc_identity_mismatch_candidate_count=1`, and `source_row_context_blocked_by_doc_identity_mismatch=1`. All 31 candidates are rejected as `missing_validated_required_axes_after_tool`; the single doc-identity mismatch is fail-closed and now reproducible at candidate granularity through non-empty `tool_candidates.source_row_context_source_atom_id` and `tool_candidates.source_row_context_doc_id`.
- Boundary measurement: `latest.json`, `latest_text_gold.json`, and `runs.jsonl` retained their pre-run hashes; protected eval namespaces had no tracked diff. Report and RunStore scans found no raw prompt/response/tool payload, authorization, API key, or secret strings. RunStore guardrails record forbidden `title`, `workbook_id`, and `workbook_version_id` as seen but not used (`forbidden_input_fields_used=[]`, `forbidden_input_fields_rejected=[]`); candidate `input_fields_used_json` is source-owned locator/date/display metadata. External archive files were not profiled or used.
- Measurement conclusion: the candidate surface is measurable and reproducible as a non-production diagnostic surface, but it does not break the gold strict plateau. The next checkpoint should target same-candidate source-native axis co-location and candidate acceptance evidence, not official metrics, promotion, gate loosening, raw XLSX/PDF query-time parsing, formula/normalized-value shortcuts, title/file shortcuts, or protected-surface mutation.
<!-- xlsx_pdf_source_native_candidate_surface_rebuild_20260620:measurements-entry:end -->

<!-- selected_evidence_failure_decomposition_20260618:measurements-entry:start -->
### selected_evidence_failure_decomposition_20260618

- Scope: report-only diagnosis of why selected-evidence-local-LLM performance remains low on the six-row text-gold route-selected SourceAtom/EvidenceBundle lane after answer discipline. `bounded_evidence_gate_v1`, gold/silver/qrels/labels/expected/denominator/source-registry/current/latest/runs surfaces, production routing, official metrics, raw prompt/response payloads, and external archive indexing stayed closed.
- Fresh report-only run: `reports/rag_eval/actual_rag_eval_selected_evidence_failure_decomposition_nonprod_20260618/report.json`; emitted files=`["report.json"]`; no `--append-registry` or `--write-latest`; local composer flags used `--local-llm-composer-backend openai-compatible --local-llm-composer-base-url http://localhost:8081/v1 --local-llm-composer-model gemma4-e2b-local`; the completed report records local LLM generation metadata plus Weaviate reachability and invocation.
- Frozen comparison: the final gate result matches the frozen answer-discipline v4 report: allowed/abstained `3/3`, unsupported-before/after `0.5/0.0`, citation_supported `6`, local_llm_acceptance_rate `0.333333`, and clean local LLM gate-blocked `2`. It remains above the frozen query-understanding local baseline (`1/5`) and deterministic query-understanding baseline (`2/4`), below the frozen current-best deterministic baseline (`5/1`), and separate from XLSX/PDF axis-v2 diagnostics (`5/24` gold, `7/493` silver).
- Failure decomposition: retrieval_empty=`0`, selected_evidence_absent=`0`, selected_evidence_anchor_missing=`3`, selected_evidence_value_present_anchor_missing=`3`, gate_blocked_clean_local_llm=`2`, pre_fallback_true_insufficient_evidence=`2`, answer_discipline_overexpansion=`0`, citation_id_mismatch_or_missing=`0`, final-gate true_insufficient_evidence=`3`, corpus_coverage_suspected_missing=`0`, and post-run expected-evidence diagnostics used=`false`.
- Follow-up gate-aligned package measurement: `reports/rag_eval/actual_rag_eval_selected_evidence_gate_aligned_fallback_nonprod_20260618/report.json`; emitted files=`["report.json"]`; no `--append-registry` or `--write-latest`; same explicit local composer flags for `gemma4-e2b-local`. The implementation keeps `bounded_evidence_gate_v1` unchanged and accepts a deterministic source-native fallback only after the fallback validates as sufficient and `allow_answer` under the same gate.
- Follow-up delta versus the decomposition report: allowed/abstained `3/3 -> 4/2`, sufficient/insufficient packages `3/3 -> 4/2`, unsupported-before-gate `0.5 -> 0.333333`, unsupported-after-gate `0.0 -> 0.0`, citation_supported `6 -> 6`, selected_evidence_anchor_missing `3 -> 2`, selected_evidence_value_present_anchor_missing `3 -> 2`, gate_blocked_clean_local_llm `2 -> 1`, and final-gate true_insufficient_evidence `3 -> 2`.
- Follow-up local composer diagnostics: `local_llm_status_counts={"answer_discipline_deterministic_fallback":2,"gate_aligned_deterministic_fallback":1,"generated":1,"unsupported_or_empty_deterministic_fallback":2}`, `local_llm_gate_aligned_fallback_count=1`, `local_llm_clean_answer_gate_blocked_count=1`, `local_llm_acceptance_rate=0.166667`, and `local_llm_fallback_reason_counts={"answer_discipline_true_insufficient_evidence":2}`. The measured fixed row is `text_namu_v2_0077`; `text_namu_v2_0014` remains missing query anchors and `text_namu_v2_0084` remains blocked by missing entity-anchor alignment.
- Dataset sufficiency and boundary result: `gold_rows_checked=6`, `silver_rows_checked=0`, source_families_observed=`{"TEXT":6}`, query_shape_counts=`{"entity_fact":1,"explanatory":1,"numeric_date_fact":2,"page_section_lookup":2}`, evidence_availability_counts=`{"selected_evidence_present":6}`, composer_outcome_counts moved `{"local_llm_accepted":2,"local_llm_fallback":4} -> {"local_llm_accepted":1,"local_llm_fallback":5}`. This is a dataset/composer diagnostic split; the separate final-gate outcome moved `3/3 -> 4/2` allowed/abstained because one row used the gate-aligned fallback. coverage_gaps=`[]`, external_archive_recommended=`false`, external_archive_used=`false`.
- Overfit/heuristic audit: no gold/expected/qrels/labels/IDs were used as runtime inputs; configured rule-surface scan `configured_rule_surface_scan_v1` found `rule_surface_count=0`, so no row-specific, dataset-specific, or literal case-term configured rules were detected. Remaining rule-based components are generic diagnostics only: answer-focus terms, `bounded_evidence_gate_v1`, and the source-family/query-shape classifier.
- Measurement conclusion: the current six-row text slice still does not justify external archive candidate inspection. The follow-up repair shows one bounded package-assembly gain for already-retrieved selected evidence, while the remaining failures still require general selected-evidence/retrieval anchor work under the unchanged gate, not corpus expansion, row-specific fixes, protected eval mutation, raw payload logging, or gate loosening.
<!-- selected_evidence_failure_decomposition_20260618:measurements-entry:end -->

<!-- selected_evidence_local_llm_answer_discipline:measurements-entry:start -->
### selected_evidence_local_llm_answer_discipline

- Scope: non-production selected-evidence-local-LLM answer discipline over the six-row text-gold route-selected SourceAtom/EvidenceBundle lane only. The local LLM remains readability-only; `bounded_evidence_gate_v1`, protected gold/silver/qrels/labels/expected/denominator/source-registry/current/latest/runs surfaces, production routing, and official metrics stayed closed.
- Fresh report-only run: `reports/rag_eval/actual_rag_eval_selected_evidence_local_llm_answer_discipline_nonprod_20260617/report.json`; emitted files=`["report.json"]`; no `--append-registry` or `--write-latest`; `official_metric=false`; `non_production=true`; `protected_namespaces_touched=[]`; raw prompt/response payload flags remain false.
- Code-level measurement surface: `selected-evidence-local-llm-v1` now records diagnostic-only answer-discipline statuses and fallback reasons using query text plus selected evidence only, with input policy `query_text_selected_evidence_answer_only_no_gold_qrels_labels_ids_or_baseline`.
- Local LLM discipline measurement: `local_llm_status_counts={"answer_discipline_deterministic_fallback":2,"generated":2,"unsupported_or_empty_deterministic_fallback":2}`, `local_llm_acceptance_rate=0.333333`, `local_llm_accepted_clean_count=2`, and `local_llm_fallback_reason_counts={"answer_discipline_true_insufficient_evidence":2}`. Final discipline counts were `{"clean_supported":2,"local_llm_rejected_then_deterministic_overexpanded":2,"true_insufficient_evidence":2}`; `answer_overexpansion_count_diagnostic=2`, `local_llm_rejected_then_deterministic_overexpanded_count=2`, `unsupported_extra_detail_count=0`, `query_irrelevant_supported_detail_count=0`, `citation_id_mismatch_or_missing_count=0`, and `anchor_morphology_false_negative_count=0`. The two accepted local-LLM rows were clean by answer discipline but blocked by the unchanged gate; the provisional successes came through deterministic fallback paths.
- Delta vs frozen local-LLM baseline `actual_rag_eval_query_understanding_local_llm_quality_check_nonprod_20260617`: allowed `1/6 -> 3/6`, insufficient `5/6 -> 3/6`, unsupported-before-gate `0.833333 -> 0.5`, unsupported-after-gate `0.0 -> 0.0`, and provisional judged/E2E answer accuracy `0.166667 -> 0.333333`.
- Comparison boundary: this is a diagnostic count improvement over the frozen query-understanding local-LLM baseline and frozen deterministic-query baseline `actual_rag_eval_query_understanding_deterministic_quality_check_nonprod_20260617`, but those baselines used the query-anchor classifier and are not a clean composer-only contrast. The frozen current-best deterministic baseline remains stronger (`5/6` allowed and provisional judged/E2E `0.5`). Do not treat this as official metric movement, production readiness, product success, or a reason to loosen the evidence gate.
<!-- selected_evidence_local_llm_answer_discipline:measurements-entry:end -->
...
- Scope: non-production route-selected Weaviate normal-query formulation only. The broader agentic loop stayed closed (`--agentic-planner-mode off`), and the evidence gate, selected-evidence composer, source registry, gold/qrels/labels/expected/denominator surfaces, production routing, and official metrics were not changed.
- Code-level measurement surface: `weaviate_query_reformulation.version=weaviate_query_reformulation_v3`, `input_policy=query_text_only_bounded_token_number_punctuation_anchor_variants_no_gold_qrels_labels_ids_or_baseline`, max variants=`8`, merge policy=`bounded_round_robin_query_variant_rank_v1`, and item-level `normalization_policies=["korean_sino_number_normalization","punctuation_normalization","query_text_only_content_anchor_compaction"]` for the residual query. Static English aliasing and tail-first collision rewrites are removed from the normal path.
- Test measurement: RED tests first failed on English alias leakage (`Adversary`, `X-Men`, `Project`), `X Men -> X-Men` special hyphenation, plain non-numeric Korean query compaction, numeric alias replacement inside unrelated Korean substrings, stale selected-evidence report narrative, and run-local memory reuse when `missing_query_anchors` were not covered by memory evidence. Focused tests now pass for v3 content-anchor variants, synthetic fake retrieval, token-boundary numeric replacement, selected-evidence report narrative, and deferred agentic tool/LLM/memory execution.
- Fresh six-row measurement: `reports/rag_eval/actual_rag_eval_query_formulation_v3_agentic_guard_nonprod_20260614_v4/report.json`; emitted files=`["report.json"]`; latest pointers moved to this run. Weaviate readiness was 200 before the run.
- Live retrieval measurement: `weaviate_query_reformulation.enabled=true`, expanded_item_count=`1`, query_variant_count_avg=`1.5`, query_count_per_item=`4.833333`, same_doc_residual_query_count=`8`, same_doc_residual_added_count=`3`, duplicate collapse removed=`19`, local corpus scan/FAISS/SearchUnit/SearchView false for same-doc residual. The residual query variants are query-text-only Korean variants: original, `엑스맨 97...`, `엑스맨 '97...`, and `엑스맨 '97 등장인물 목록 애드버서리`.
- Live gate measurement: allowed/blocked stayed `5/1`; sufficient/insufficient stayed `5/1`; unsupported-before/after stayed `0.166667/0.0`; citation_supported is `8`; retrieved-context-only diagnostic citations stayed `0`; selected-evidence citation precision stayed `1.0`; `metric_continuity_checkpoint.selected_evidence_gate_outcome_preserved=true`.
- Residual measurement: `text_namu_v2_0014` no longer retrieves the target Korean Adversary line after static alias removal. The unchanged evidence gate still blocks with `evidence_package_status=insufficient`, `abstention_reason=insufficient_evidence`, and missing query anchors `구십칠|애드버서리`. Measurement conclusion: v3 reduces heuristic risk and query volume but does not preserve the v2 retrieval gain; broader-loop readiness remains false.
<!-- actual_rag_eval_query_formulation_v3_agentic_guard_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_agentic_loop_review_nonprod:measurements-entry:start -->
### actual_rag_eval_agentic_loop_review_nonprod

- Scope: non-production broader-loop readiness review only. The active RAG boundary remains route-selected retrieval plus deterministic selected-evidence composer plus evidence gate; the review does not execute retrieval, tools, memory, or LLM retry.
- Code-level measurement surface: `agentic_loop_review` records `planner_mode`, `planner_version`, bounded action counts, expected and executed query/tool/LLM/memory counts, `gate_delta`, `quality_improvement_measured`, `live_text_gold_metric_measured`, `planner_memory_tool_llm_retry_loop_status`, required evidence before broader-loop opening, and recommendation=`keep_broader_agent_loop_closed_continue_bounded_execute_once_evidence_gate_checkpoints`.
- Guardrail measurement: the review is `review_only`, `official_metric=false`, and `heuristic_risk_class=diagnostic_probe_only`; it requires evidence-gate validation for all actions and one action per failed row. Validator coverage rejects broader-loop ready/opened claims, production routing, raw prompt/response payloads, gate loosening, retrieved-context-only citation promotion, and gold/qrels/label/expected/denominator mutation.
- Test measurement: RED report test failed first on missing `agentic_loop_review`, then passed and proved a fake execute-once quality delta is recorded without opening the broader loop. The new guardrail regression passed (`8 passed, 214 deselected`); planner-focused slice passed (`62 passed, 160 deselected`); full metric-generation file passed on rerun (`222 passed` after an earlier 180s timeout); current profile passed (`133 passed, 330 deselected`); `py_compile` passed; `git diff --check` passed aside from LF/CRLF warnings.
- Fresh six-row measurement: `reports/rag_eval/actual_rag_eval_agentic_loop_review_nonprod_20260613/report.json`; emitted files=`["report.json"]`; latest pointers now reference the run. Before the successful run, Weaviate was live but not ready while vector caches loaded; after readiness flipped healthy, the route-selected report completed.
- Live gate measurement: allowed/blocked stayed `5/1`; sufficient/insufficient stayed `5/1`; unsupported-before/after stayed `0.166667/0.0`; citation_supported=`8`; retrieved-context-only diagnostic citations=`0`; selected-evidence citation precision=`1.0`; query_count_per_item=`5.5`. `metric_continuity_checkpoint.selected_evidence_gate_outcome_preserved=true` and `residual_resolution_assessment=residual_remains_general_retrieval_limitation`.
- Live planner measurement: `agentic_planner_execute_once.planner_action_counts={"run_local_memory_reuse":1}`, failure_class_counts=`{"missing_query_anchor":1}`, expected_memory_lookup_count=`1`, executed query/tool/LLM/memory counts all `0`, `gate_delta.allowed_answer_count_delta=0`, and `agentic_loop_review.bounded_action_evidence.quality_improvement_measured=false`.
- Measurement conclusion: planner/memory/tool-use/LLM-retry loops are measurable as bounded one-action checkpoints, but this live text-gold checkpoint produced no quality delta. The broader agent loop is not ready; the remaining work is a general retrieval/query formulation improvement under the unchanged gate plus human-approved official metric/denominator policy, not another eval-row shortcut.
<!-- actual_rag_eval_agentic_loop_review_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_agentic_planner_run_local_memory_nonprod:measurements-entry:start -->
### actual_rag_eval_agentic_planner_run_local_memory_nonprod

- Scope: non-production execute-once planner checkpoint with one run-local selected-evidence memory reuse action only. The active RAG boundary remains route-selected retrieval plus deterministic selected-evidence composer plus evidence gate; memory reuse is post-gate and accepted only through the unchanged gate.
- Code-level measurement surface: `agentic_planner_execute_once` now records `run_local_memory_reuse` in planner action counts, `planner_expected_memory_lookup_count`, `planner_memory_lookup_count_executed`, per-decision `memory_lookup_count_executed`, `memory_input_policy`, source query hash, and memory evidence IDs, plus row-level `agentic_planner_run_local_memory` metadata.
- Guardrail measurement: the memory bank is derived only from same-run allowed rows and their selected SourceAtom/EvidenceBundle evidence. It uses query text plus source-derived evidence text/identity, not query_id/row_id/target_id, expected answer/evidence, qrels, labels, baseline_topk, legacy outputs, row-specific alias tables, raw prompt/response payloads, retrieval, tools, or LLM retry.
- Test measurement: the run-local memory RED-to-GREEN test measured fake-row gate improvement from allowed/blocked `1/1 -> 2/0` with `planner_action_counts={"run_local_memory_reuse":1}`, `planner_failure_class_counts={"insufficient_evidence":1}`, `planner_expected_memory_lookup_count=1`, `planner_memory_lookup_count_executed=1`, and zero retrieval/tool/LLM execution. Planner-focused slice passed (`54 passed, 160 deselected`); full actual-RAG metric-generation file passed (`214 passed`); current profile passed (`133 passed, 322 deselected`).
- Fresh six-row measurement status: blocked by unavailable local Weaviate at `http://localhost:8080`; attempted run `actual_rag_eval_agentic_planner_run_local_memory_nonprod_20260613` produced no `report.json`, no report directory, and no latest-pointer update.
- Measurement conclusion: run-local selected-evidence memory can improve a gated fake row without crossing evidence or eval-row guardrails. Live text-gold improvement remains unmeasured, so readiness is code-level only; broader agent-loop review remains next and should not open production or official metrics.
<!-- actual_rag_eval_agentic_planner_run_local_memory_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_agentic_planner_llm_retry_redesign_nonprod:measurements-entry:start -->
### actual_rag_eval_agentic_planner_llm_retry_redesign_nonprod

- Scope: non-production execute-once planner checkpoint with one selected-evidence LLM rewrite action only. The active RAG boundary remains route-selected retrieval plus deterministic selected-evidence composer plus evidence gate; the LLM retry is post-gate and accepted only through the unchanged gate.
- Code-level measurement surface: `agentic_planner_execute_once` now records expected/executed LLM retry counts, `planner_execution.llm_retry_executed`, `planner_execution.llm_retry_count_executed`, per-decision `llm_retry_input_policy`, and row-level `agentic_planner_llm_retry` metadata with selected-evidence IDs, previous-answer preview hash, retry prompt hash, response hash, and accepted/rejected status.
- Guardrail measurement: the retry prompt is built from query text, selected evidence, public gate diagnostics, and bounded previous-answer preview only. It excludes query_id/row_id/target_id, expected answer/evidence, qrels, labels, baseline_topk, and legacy outputs; raw prompt/response payload flags remain false; retrieval/tool counts remain zero for this action; retrieved-context-only citations are never promoted.
- Test measurement: the bounded LLM retry RED-to-GREEN test measured fake-row gate improvement from allowed/blocked `0/1 -> 1/0` with `planner_action_counts={"selected_evidence_llm_rewrite":1}`, `planner_failure_class_counts={"unsupported_generation":1}`, `planner_expected_llm_retry_count=1`, `llm_retry_count_executed=1`, and zero retrieval/tool execution. Planner-focused slice passed (`53 passed, 160 deselected`); full actual-RAG metric-generation file passed (`213 passed`); current profile passed (`133 passed, 321 deselected`).
- Fresh six-row measurement status: blocked by unavailable local Weaviate at `http://localhost:8080`; attempted run `actual_rag_eval_agentic_planner_llm_retry_redesign_nonprod_20260613` produced no `report.json`, no report directory, and no latest-pointer update.
- Measurement conclusion: bounded selected-evidence LLM retry can improve a gated fake unsupported-generation row without crossing evidence or eval-row guardrails. Live text-gold improvement remains unmeasured, so readiness is code-level only and run-local memory remains unopened.
<!-- actual_rag_eval_agentic_planner_llm_retry_redesign_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_agentic_planner_tool_use_narrow_nonprod:measurements-entry:start -->
### actual_rag_eval_agentic_planner_tool_use_narrow_nonprod

- Scope: non-production execute-once planner checkpoint with narrow PDF/XLSX source-derived locator tool execution only. The active RAG boundary remains route-selected retrieval plus deterministic selected-evidence composer plus evidence gate; the tool action is post-gate, bounded to one source-derived locator context, and then reuses the same composer/gate.
- Code-level measurement surface: `agentic_planner_execute_once` records `planner_action_counts`, `planner_failure_class_counts`, expected and executed query/tool/LLM retry counts, `planner_execution`, `gate_before`, `gate_after`, `gate_delta`, per-decision execution status, and `agentic_planner_tool_use` metadata on rows where a source-derived locator tool is executed.
- Guardrail measurement: tool execution reads only retrieved-context locator fields owned by the source context and matching PDF/XLSX source family. It uses no query_id/row_id/target_id, expected answer/evidence, qrels, labels, baseline_topk, or legacy outputs; writes no raw prompt/response payloads; performs no LLM retry; does not loosen the evidence gate; and does not promote retrieved-context-only citations.
- Test measurement: the PDF locator RED test initially selected `query_text_only_reformulation`; after implementation it measured fake-row gate improvement from allowed/blocked `0/1 -> 1/0` with `planner_action_counts={"pdf_locator_tool":1}`, `planner_failure_class_counts={"tool_required_pdf":1}`, `tool_call_count_executed=1`, and no extra retrieval queries. Planner-focused slice passed (`52 passed, 160 deselected`); full actual-RAG metric-generation file passed (`212 passed`); current profile passed (`133 passed, 320 deselected`).
- Fresh six-row measurement status: blocked by unavailable local Weaviate at `http://localhost:8080`; attempted run `actual_rag_eval_agentic_planner_tool_use_narrow_nonprod_20260613` produced no `report.json`, no report directory, and no latest-pointer update.
- Execute-once measurement conclusion: bounded source-derived tool execution can improve a gated fake PDF row without crossing eval-row or citation guardrails. Live text-gold improvement remains unmeasured, so readiness is limited to code-level execute-once under fakes, not broader tool/memory/LLM agent loops.
<!-- actual_rag_eval_agentic_planner_tool_use_narrow_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_agentic_planner_execute_once_nonprod:measurements-entry:start -->
### actual_rag_eval_agentic_planner_execute_once_nonprod

- Scope: non-production execute-once planner checkpoint only. The active RAG boundary remains route-selected retrieval plus deterministic selected-evidence composer plus evidence gate; execute-once adds one bounded post-gate retrieval probe for executable planner actions and then reuses the same composer/gate.
- Code-level measurement surface: `agentic_planner_execute_once` records planner decision counts, executed decision count, proposed action counts, failure class counts, expected and executed query/tool/LLM retry budgets, `gate_before`, `gate_after`, `gate_delta`, `planner_execution`, `guardrail_mutation_flags`, and decision rows with query hash/preview only.
- Guardrail measurement: scorer `failure_labels` are ignored for planner selection; execute-once extra retrieval receives a sanitized item with no query_id/row_id/target_id, expected answer/evidence, qrels, labels, baseline_topk, or legacy output fields. official_metric=false; official_metric_input_rows=`0`; raw prompt/response payload flags false; tools, LLM retry, production routing, protected namespace mutation, gate loosening, and retrieved-context-only citation promotion remain false.
- Test measurement: label-leakage RED-to-GREEN test passed; execute-once RED-to-GREEN test passed and measured fake-row gate improvement from allowed/blocked `0/1 -> 1/0` after exactly one sanitized extra query. Planner-focused slice passed (`51 passed, 160 deselected`); full actual-RAG metric-generation file passed (`211 passed`).
- Fresh six-row measurement status: blocked by unavailable local Weaviate at `http://localhost:8080`; attempted run `actual_rag_eval_agentic_planner_execute_once_nonprod_20260613` produced no `report.json`, no report directory, and no latest-pointer update. The latest measurable live six-row report remains `actual_rag_eval_adversary_corpus_coverage_audit_nonprod_20260613_v3`.
- Execute-once measurement conclusion: fake-backed quality improvement is proven under unchanged evidence-gate validation; live text-gold improvement is not yet measured because the external Weaviate dependency is unavailable.
<!-- actual_rag_eval_agentic_planner_execute_once_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_agentic_planner_dry_run_nonprod:measurements-entry:start -->
### actual_rag_eval_agentic_planner_dry_run_nonprod

- Scope: non-production planner dry-run checkpoint only. The active RAG boundary remains Weaviate route-selected plus selected-evidence composer plus evidence gate; dry-run planner diagnostics run after that boundary and are report-only.
- Code-level measurement surface: `agentic_planner_dry_run` records `planner_enabled`, `planner_mode`, `planner_version`, `planner_decision_count`, `planner_action_counts`, `planner_failure_class_counts`, `planner_no_safe_action_count`, `planner_forbidden_shortcut_detected_count`, expected extra query/tool/LLM retry counts, `planner_heuristic_risk_class`, `gate_before`, `gate_after_unchanged_because_dry_run`, and closed guardrail/execution flags.
- Guardrail measurement: official_metric=false; official_metric_input_rows=`0`; raw prompt/response payload flags false; production routing and protected namespace mutation flags false; retrieved-context-only citation policy=`diagnostic_only_never_promoted`; planner execution counters for retrieval, tools, and LLM retry all false/zero.
- Test measurement: planner-focused slice passed (`49 passed, 160 deselected`); full actual-RAG metric-generation file passed (`209 passed`); the fake-Weaviate dry-run regression compares planner-off and dry-run `query_log` payloads exactly and verifies no added retrieval, no upserts, no local scan, and no forbidden eval-row fields in retrieval payloads.
- Fresh six-row measurement status: blocked by unavailable local Weaviate at `http://localhost:8080`; the attempted run `actual_rag_eval_agentic_planner_dry_run_nonprod_20260613` produced no `report.json`, no report directory, and no latest-pointer update. The latest measurable live six-row report remains `actual_rag_eval_adversary_corpus_coverage_audit_nonprod_20260613_v3`.
- Execute-once measurement conclusion: not ready. Dry-run estimates budgets and preserves the gate, but it does not execute a planner action or measure an allowed/blocked quality delta.
<!-- actual_rag_eval_agentic_planner_dry_run_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_adversary_corpus_coverage_audit_nonprod:measurements-entry:start -->
### actual_rag_eval_adversary_corpus_coverage_audit_nonprod

- Scope: non-production report-only corpus coverage audit for `text_namu_v2_0014` only. The active retrieval boundary stays Weaviate route-selected over SourceAtom/EvidenceBundle; full-index Weaviate rollback remains available; official metrics, production readiness, gold/qrels/labels/answerability/expected/denominator/source-registry/current mutation, broad planner, Memory Agent, Tool Agent, and raw prompt/response artifacts remain closed.
- Fresh report: `reports/rag_eval/actual_rag_eval_adversary_corpus_coverage_audit_nonprod_20260613_v3/report.json`; emitted files=`["report.json"]`; corpus_coverage_audit_jsonl=``; report_only_diagnostic=true; artifact_sidecar_written=false; active_retrieval_backend=`weaviate_hybrid`; active_retrieval_service_boundary=`weaviate`; collection=`SourceAtomNonprodRouteSelectedV2`; route_mode=`route_selected`; rollback_key=`weaviate_full_index_nonprod_rollback`.
- Audit target anchors: `Adversary`, `Alison Sealy-Smith`, `애드버서리`, `앨리슨 실리스미스`; audit query policy=`query_text_plus_explicit_target_anchors_report_only_after_main_run`; gold_or_qrels_mutation=false; label_or_denominator_mutation=false; official_metric_input_rows=`0`; raw_prompt_payload_written=false; raw_response_payload_written=false.
- Classification measurement: classification_counts=`{"tokenization_alias_failure":1}`; primary_classification=`tokenization_alias_failure`; corpus_presence=`corpus_present`; failure_modes=`["tokenization_alias_failure","collision"]`; source-registry target_anchor_hit=true, matching_source_atom_count=`4`, strong_target_anchor_hit_count=`2`, and query_id_match_count=`1`; active index checkpoint target_source_atom_indexed=true and source_atom_id_match_count=`4`; active main retrieval target_anchor_hit=false; active collision_count=`3`; full-index target probe target_anchor_hit=true and target_anchor_hit_count=`2`; route-selected target probe target_anchor_hit=true and target_anchor_hit_count=`2`; route-selected route_filter_sent=true.
- Gate measurement: `text_namu_v2_0014` still has evidence_package_status=`insufficient`, generated_answer=`제공된 근거만으로는 답할 수 없습니다.`, and missing query anchors `구십칠|등장인물|애드버서리`. This separates the miss as corpus-present tokenization/alias failure with collision pressure, not corpus absence or route-selected filter failure.
<!-- actual_rag_eval_adversary_corpus_coverage_audit_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_selected_evidence_same_doc_residual_checkpoint_j_nonprod:measurements-entry:start -->
### actual_rag_eval_selected_evidence_same_doc_residual_checkpoint_j_nonprod

- Scope: non-production selected-evidence follow-on checkpoint only. The active retrieval boundary stays Weaviate route-selected over SourceAtom/EvidenceBundle; full-index Weaviate rollback remains available; official metrics, production readiness, gold/qrels/labels/answerability/expected/denominator/source-registry/current mutation, broad planner, Memory Agent, Tool Agent, and raw prompt/response artifacts remain closed.
- Fresh report: `reports/rag_eval/actual_rag_eval_selected_evidence_same_doc_residual_checkpoint_j_nonprod_20260613/report.json`; emitted files=`["report.json"]`; active_retrieval_backend=`weaviate_hybrid`; active_retrieval_service_boundary=`weaviate`; collection=`SourceAtomNonprodRouteSelectedV2`; route_mode=`route_selected`; rollback_key=`weaviate_full_index_nonprod_rollback`.
- Same-document residual measurement: policy=`bounded_query_variant_same_doc_weaviate_v1`; expanded_item_count=`1`; query_count=`8`; added_count=`3`; max_docs=`2`; top_k_per_doc=`3`; candidate_generation=`weaviate_doc_id_filter_plus_query_variants_no_gold_qrels_labels_ids_or_baseline`; local corpus scan, FAISS, and SearchUnit/SearchView all false for this residual path.
- Composer/gate measurement: selected_evidence_composer_invoked=true; citation_format=`markdown-portfolio`; selected_evidence_composer_selected_evidence_count=`8`; citation_supported_count=`8`; citation_retrieved_context_only_diagnostic_count=`0`; unsupported_answer_rate_before_gate=`0.166667`; unsupported_answer_rate_after_gate=`0.0`; allowed_answer_count=`5`; unsupported_answer_blocked_count=`1`; abstained_count=`1`.
- Delta versus Checkpoint I: gate/citation outcomes unchanged at allowed/blocked `5/1`, sufficient/insufficient `5/1`, unsupported-before/after `0.166667/0.0`, citation_supported `8`, retrieved-context-only `0`; Weaviate query_count_per_item increased `4.166667 -> 5.5`.
- Remaining residual measurement: `text_namu_v2_0014` same-doc residual retrieval added three contexts from `docv_namu_v4_ffa9998ff6067ea2`, including `X-Men 97` query-variant provenance, but no retrieved context text/title contains `Adversary`, `애드버서리`, or `어드버서리`. The row remains insufficient with missing query anchors `구십칠`, `등장인물`, and `애드버서리`.
<!-- actual_rag_eval_selected_evidence_same_doc_residual_checkpoint_j_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_selected_evidence_route_query_variant_checkpoint_i_nonprod:measurements-entry:start -->
### actual_rag_eval_selected_evidence_route_query_variant_checkpoint_i_nonprod

- Scope: non-production selected-evidence follow-on checkpoint only. The active retrieval boundary stays Weaviate route-selected over SourceAtom/EvidenceBundle; full-index Weaviate rollback remains available; official metrics, production readiness, gold/qrels/labels/answerability/expected/denominator/source-registry/current mutation, broad planner, Memory Agent, Tool Agent, and raw prompt/response artifacts remain closed.
- Fresh report: `reports/rag_eval/actual_rag_eval_selected_evidence_route_query_variant_checkpoint_i_nonprod_20260613/report.json`; emitted files=`["report.json"]`; active_retrieval_backend=`weaviate_hybrid`; active_retrieval_service_boundary=`weaviate`; collection=`SourceAtomNonprodRouteSelectedV2`; route_mode=`route_selected`; rollback_key=`weaviate_full_index_nonprod_rollback`.
- Query reformulation measurement: version=`weaviate_query_reformulation_v1`; enabled=true; expanded_item_count=`1`; item_count=`6`; query_variant_count_avg=`2.166667`; max_query_variants=`8`; merge_policy=`bounded_round_robin_query_variant_rank_v1`; input_policy=`query_text_only_bounded_alias_variants_no_gold_qrels_labels_ids_or_baseline`; uses_gold/expected/qrels/labels/IDs/baseline_topk/legacy_outputs=false.
- Composer/gate measurement: selected_evidence_composer_invoked=true; citation_format=`markdown-portfolio`; selected_evidence_composer_selected_evidence_count=`8`; citation_supported_count=`8`; citation_retrieved_context_only_diagnostic_count=`0`; unsupported_answer_rate_before_gate=`0.166667`; unsupported_answer_rate_after_gate=`0.0`; allowed_answer_count=`5`; unsupported_answer_blocked_count=`1`; abstained_count=`1`.
- Delta versus Checkpoint H: gate/citation outcomes unchanged at allowed/blocked `5/1`, sufficient/insufficient `5/1`, unsupported-before/after `0.166667/0.0`, citation_supported `8`, retrieved-context-only `0`; Weaviate query_count_per_item increased `3.0 -> 4.166667` from the bounded variant probes.
- Remaining residual measurement: `text_namu_v2_0014` variants included `X-Men 97 ... Adversary` and `X-Men '97 ... Adversary`; retrieved contexts now include an `엑스맨 '97` passage, but no retrieved context text/title contains `Adversary`, `애드버서리`, or `어드버서리`. The row remains insufficient with missing query anchors `구십칠` and `애드버서리`.
<!-- actual_rag_eval_selected_evidence_route_query_variant_checkpoint_i_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_selected_evidence_query_focus_repair_checkpoint_h_nonprod:measurements-entry:start -->
### actual_rag_eval_selected_evidence_query_focus_repair_checkpoint_h_nonprod

- Scope: non-production selected-evidence follow-on checkpoint only. The active retrieval boundary stays Weaviate route-selected over SourceAtom/EvidenceBundle; full-index Weaviate rollback remains available; official metrics, production readiness, gold/qrels/labels/answerability/expected/denominator/source-registry/current mutation, broad planner, Memory Agent, Tool Agent, and raw prompt/response artifacts remain closed.
- Fresh report: `reports/rag_eval/actual_rag_eval_selected_evidence_query_focus_repair_checkpoint_h_nonprod_20260612/report.json`; emitted files=`["report.json"]`; active_retrieval_backend=`weaviate_hybrid`; active_retrieval_service_boundary=`weaviate`; collection=`SourceAtomNonprodRouteSelectedV2`; route_mode=`route_selected`; rollback_key=`weaviate_full_index_nonprod_rollback`.
- Composer/gate measurement: selected_evidence_composer_invoked=true; citation_format=`markdown-portfolio`; selected_evidence_composer_selected_evidence_count=`8`; citation_supported_count=`8`; citation_retrieved_context_only_diagnostic_count=`0`; unsupported_answer_rate_before_gate=`0.166667`; unsupported_answer_rate_after_gate=`0.0`; allowed_answer_count=`5`; unsupported_answer_blocked_count=`1`; abstained_count=`1`.
- Delta versus Checkpoint G deterministic markdown enforce: allowed rows `3 -> 5`; blocked rows `3 -> 1`; sufficient evidence packages `3 -> 5`; insufficient evidence packages `3 -> 1`; unsupported-before-gate `0.5 -> 0.166667`; unsupported-after-gate unchanged at `0.0`; retrieved-context-only citation count unchanged at `0`.
- Remaining residual measurement: `text_namu_v2_0014` remains insufficient with missing query anchors `구십칠` and `애드버서리`; current retrieved contexts do not contain those anchors, so the next repair belongs to route-selected retrieval/query formulation, not gold mutation or citation promotion.
<!-- actual_rag_eval_selected_evidence_query_focus_repair_checkpoint_h_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_selected_evidence_portfolio_sidecar_checkpoint_g_nonprod:measurements-entry:start -->
### actual_rag_eval_selected_evidence_portfolio_sidecar_checkpoint_g_nonprod

- Scope: non-production portfolio experiment Checkpoint G only. The active retrieval boundary stays Weaviate route-selected over SourceAtom/EvidenceBundle; full-index Weaviate rollback remains available; official metrics, production readiness, gold/qrels/labels/answerability/expected/denominator/source-registry/current mutation, broad planner, Memory Agent, Tool Agent, and raw prompt/response artifacts remain closed.
- Fresh report and sidecar: `reports/rag_eval/actual_rag_eval_selected_evidence_portfolio_sidecar_checkpoint_g_nonprod_20260612_v2/report.json` and `reports/rag_eval/actual_rag_eval_selected_evidence_portfolio_sidecar_checkpoint_g_nonprod_20260612_v2/portfolio_experiment_summary.md`; emitted files=`["portfolio_experiment_summary.md","report.json"]`; sidecar flag=true; portfolio comparison lane_count=`9`.
- Boundary measurement: active_retrieval_backend=`weaviate_hybrid`; active_retrieval_service_boundary=`weaviate`; collection=`SourceAtomNonprodRouteSelectedV2`; route_mode=`route_selected`; rollback_key=`weaviate_full_index_nonprod_rollback`; raw_prompt_payload_written=false; raw_response_payload_written=false; official_metric_input_rows=`0`; protected_namespaces_touched=`[]`.
- Sidecar measurement: required sections present: Answer Diff, Citation Diff, Gate Before/After, unsupported answer blocked count, abstain count, citation precision against selected evidence, retrieved-context-only citation count, and Residual Failure Taxonomy. Forbidden payload checks passed: no `"prompt":`, no `"raw_response":`, no `Return exactly one JSON`, and no `Payload:` marker in the sidecar.
- Metric carry-forward: deterministic selected-evidence enforce remains allowed/blocked=`3/3`, unsupported-before/after=`0.5/0.0`, citation_supported=`8`, retrieved_context_only_diagnostic=`0`, selected-evidence citation precision=`1.0`; extractive diagnostic remains citation_supported=`9`, retrieved_context_only_diagnostic=`40`, selected-evidence citation precision=`0.183673`; local LLM remains allowed/blocked=`1/5`.
<!-- actual_rag_eval_selected_evidence_portfolio_sidecar_checkpoint_g_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_selected_evidence_comparison_checkpoint_f_nonprod:measurements-entry:start -->
### actual_rag_eval_selected_evidence_comparison_checkpoint_f_nonprod

- Scope: non-production portfolio experiment Checkpoint F only. The active retrieval boundary stays Weaviate route-selected over SourceAtom/EvidenceBundle; full-index Weaviate rollback remains available through `ai/eval/configs/weaviate_full_index_nonprod_rollback.json`; official metrics, production readiness, gold/qrels/labels/answerability/expected/denominator/source-registry/current mutation, broad planner, Memory Agent, Tool Agent, and raw prompt/response artifacts remain closed.
- Fresh final report: `reports/rag_eval/actual_rag_eval_selected_evidence_comparison_checkpoint_f_report_nonprod_20260612/report.json`; emitted files=`["report.json"]`; active_retrieval_backend=`weaviate_hybrid`; active_retrieval_service_boundary=`weaviate`; collection=`SourceAtomNonprodRouteSelectedV2`; route_mode=`route_selected`; rollback_key=`weaviate_full_index_nonprod_rollback`; portfolio_comparison_schema=`actual_rag_eval.portfolio_experiment_comparison.v1`; lane_count=`9`; portfolio_experiment_sidecar_written=false.
- Matrix reports: `actual_rag_eval_selected_evidence_comparison_checkpoint_f_extractive_diagnostic_nonprod_20260612`, `actual_rag_eval_selected_evidence_comparison_checkpoint_f_extractive_enforce_nonprod_20260612`, `actual_rag_eval_selected_evidence_comparison_checkpoint_f_deterministic_diagnostic_nonprod_20260612`, `actual_rag_eval_selected_evidence_comparison_checkpoint_f_deterministic_compact_nonprod_20260612`, `actual_rag_eval_selected_evidence_comparison_checkpoint_f_deterministic_evidence_id_nonprod_20260612`, `actual_rag_eval_selected_evidence_comparison_checkpoint_f_deterministic_source_locator_nonprod_20260612`, `actual_rag_eval_selected_evidence_comparison_checkpoint_f_local_llm_enforce_nonprod_20260612`, prior `actual_rag_eval_selected_evidence_retry_checkpoint_e_nonprod_20260612`, and current deterministic markdown enforce in the final report.
- Extractive-v1 measurement: diagnostic unsupported-before/after=`0.666667/0.666667`, allowed=`2`, would-block=`4`, citation_supported=`9`, retrieved_context_only_diagnostic=`40`, selected-evidence citation precision=`0.183673`; enforce unsupported-before/after=`0.666667/0.0`, allowed=`2`, blocked=`4`, abstained=`4`, citation_supported=`9`, retrieved_context_only_diagnostic=`40`.
- Deterministic selected-evidence measurement: diagnostic unsupported-before/after=`0.5/0.5`, would-block=`3`, citation_supported=`8`, retrieved_context_only_diagnostic=`0`; all enforced formatter variants (`compact`, `evidence-id`, `source-locator`, `markdown-portfolio`) had allowed/blocked=`3/3`, unsupported-before/after=`0.5/0.0`, citation_supported=`8`, retrieved_context_only_diagnostic=`0`, selected-evidence citation precision=`1.0`.
- Local LLM measurement: fresh local LLM enforce had local helper available, allowed/blocked=`1/5`, unsupported-before/after=`0.833333/0.0`, citation_supported=`6`, retrieved_context_only_diagnostic=`0`, selected-evidence citation precision=`1.0`; bounded retry had the same gate/citation result with retry attempts=`4`, accepted=`0`, rejected=`4`.
- Comparison deltas: selected-evidence lanes versus extractive diagnostic recorded answer diffs `6/6`, citation diffs `6/6`, retrieved-context-only citation delta `40 -> 0`, and final unsupported-after-gate `0.0` in enforce mode. Deterministic selected-evidence remains the strongest active experiment path: it keeps citation precision at `1.0` and allows `3/6`, while local LLM allows only `1/6`.
<!-- actual_rag_eval_selected_evidence_comparison_checkpoint_f_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_selected_evidence_retry_checkpoint_e_nonprod:measurements-entry:start -->
### actual_rag_eval_selected_evidence_retry_checkpoint_e_nonprod

- Scope: non-production portfolio experiment Checkpoint E only. The active retrieval boundary stays Weaviate route-selected over SourceAtom/EvidenceBundle; full-index Weaviate rollback remains available through `ai/eval/configs/weaviate_full_index_nonprod_rollback.json`; official metrics, production readiness, gold/qrels/labels/answerability/expected/denominator/source-registry/current mutation, broad planner, Memory Agent, Tool Agent, and raw prompt/response artifacts remain closed.
- Fresh report: `reports/rag_eval/actual_rag_eval_selected_evidence_retry_checkpoint_e_nonprod_20260612/report.json`; emitted files=`["report.json"]`; active_retrieval_backend=`weaviate_hybrid`; active_retrieval_service_boundary=`weaviate`; collection=`SourceAtomNonprodRouteSelectedV2`; route_mode=`route_selected`; rollback_key=`weaviate_full_index_nonprod_rollback`; generator provider=`selected-evidence-local-llm-v1`; citation_format=`markdown-portfolio`; retry_mode=`bounded-once`.
- Retry measurement: selected_evidence_composer_retry_enabled=true; max_count_per_item=`1`; attempt_count=`4`; accepted_count=`0`; rejected_count=`4`; error_count=`0`; status_counts=`{"not_triggered":2,"rejected_gate_insufficient":4}`; retry_input_policy=`query_text_selected_evidence_missing_query_focus_anchors_previous_bounded_answer_preview_only_no_gold_qrels_labels_ids_or_baseline`; retry_raw_prompt_payload_written=false; retry_raw_response_payload_written=false.
- Local LLM composer measurement: local_llm_generation_available=true; local_llm_status_counts=`{"generated":5,"unsupported_or_empty_deterministic_fallback":1}`; local_llm_blockers=`[]`; local_llm_prompt_payload_written=false; local_llm_raw_response_payload_written=false; prompt/response surfaces are persisted only as hashes, bounded answer previews, and backend/status metadata.
- Evidence gate enforce measurement: sufficient=`1`; insufficient=`5`; allowed_answer_count=`1`; abstained_count=`5`; unsupported_answer_blocked_count=`5`; unsupported_answer_rate_before_gate=`0.833333`; unsupported_answer_rate_after_gate=`0.0`; citation_supported_count=`6`; citation_retrieved_context_only_diagnostic_count=`0`; citation_wrong_target_count=`0`; citation_missing_target_count=`0`; citation_unsupported_text_count=`0`.
- Delta versus Checkpoint D local LLM composer: unsupported_answer_rate_after_gate stays `0.0`; unsupported_answer_blocked_count stays `5`; allowed_answer_count stays `1`; citation_supported_count stays `6`; retrieved-context-only citation count stays `0`; retry attempts were all rejected by the evidence gate, so this is an artifact-safe fail-closed retry implementation but not an answer-quality improvement.
- Residual row taxonomy: `text_namu_v2_0005` allowed with retry not triggered because the evidence gate allowed the answer; `text_namu_v2_0012` blocked as `insufficient_evidence` with retry not triggered because the initial local answer was unsupported/empty and used deterministic fallback; `text_namu_v2_0014`, `text_namu_v2_0017`, `text_namu_v2_0077`, and `text_namu_v2_0084` retried once and were rejected as still gate-insufficient or missing entity/query anchors.
<!-- actual_rag_eval_selected_evidence_retry_checkpoint_e_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_selected_evidence_local_llm_checkpoint_d_nonprod:measurements-entry:start -->
### actual_rag_eval_selected_evidence_local_llm_checkpoint_d_nonprod

- Scope: non-production portfolio experiment Checkpoint D only. The active retrieval boundary stays Weaviate route-selected over SourceAtom/EvidenceBundle; full-index Weaviate rollback remains available through `ai/eval/configs/weaviate_full_index_nonprod_rollback.json`; official metrics, production readiness, gold/qrels/labels/answerability/expected/denominator/source-registry/current mutation, broad planner, Memory Agent, Tool Agent, and raw prompt/response artifacts remain closed.
- Fresh report: `reports/rag_eval/actual_rag_eval_selected_evidence_local_llm_checkpoint_d_nonprod_20260612/report.json`; emitted files=`["report.json"]`; active_retrieval_backend=`weaviate_hybrid`; active_retrieval_service_boundary=`weaviate`; collection=`SourceAtomNonprodRouteSelectedV2`; route_mode=`route_selected`; rollback_key=`weaviate_full_index_nonprod_rollback`; generator provider=`selected-evidence-local-llm-v1`; citation_format=`markdown-portfolio`.
- Local LLM composer measurement: local_llm_generation_available=true; actual_generation_model_used=true; local_llm_composer_fallback_used=true; local_llm_status_counts=`{"generated":5,"unsupported_or_empty_deterministic_fallback":1}`; local_llm_blockers=`[]`; raw_prompt_payload_written=false; raw_response_payload_written=false; prompt/response surfaces are persisted only as hashes, bounded answer previews, and backend/status metadata.
- Evidence gate enforce measurement: sufficient=`1`; insufficient=`5`; allowed_answer_count=`1`; abstained_count=`5`; unsupported_answer_blocked_count=`5`; unsupported_answer_rate_before_gate=`0.833333`; unsupported_answer_rate_after_gate=`0.0`; citation_supported_count=`6`; citation_retrieved_context_only_diagnostic_count=`0`; citation_wrong_target_count=`0`; citation_missing_target_count=`0`; citation_unsupported_text_count=`0`.
- Delta versus Checkpoint C deterministic formatter: unsupported_answer_rate_after_gate stays `0.0`; unsupported_answer_blocked_count changes `3 -> 5`; allowed_answer_count changes `3 -> 1`; citation_supported_count changes `8 -> 6`; retrieved-context-only citation count stays `0`. Interpretation: the optional local LLM path is available and artifact-safe, but is not yet better than the deterministic composer under the evidence gate.
- Residual row taxonomy: `text_namu_v2_0005` allowed; `text_namu_v2_0012` blocked as `insufficient_evidence`; `text_namu_v2_0014`, `text_namu_v2_0017`, `text_namu_v2_0077`, and `text_namu_v2_0084` blocked as `missing_entity_anchor`.
<!-- actual_rag_eval_selected_evidence_local_llm_checkpoint_d_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_selected_evidence_citation_formatter_checkpoint_c_nonprod:measurements-entry:start -->
### actual_rag_eval_selected_evidence_citation_formatter_checkpoint_c_nonprod

- Scope: non-production portfolio experiment Checkpoint C only. The active retrieval boundary stays Weaviate route-selected over SourceAtom/EvidenceBundle; full-index Weaviate rollback remains available through `ai/eval/configs/weaviate_full_index_nonprod_rollback.json`; official metrics, production readiness, gold/qrels/labels/answerability/expected/denominator/source-registry/current mutation, broad planner, Memory Agent, Tool Agent, and raw prompt/response artifacts remain closed.
- Fresh report: `reports/rag_eval/actual_rag_eval_selected_evidence_citation_formatter_checkpoint_c_nonprod_20260612/report.json`; emitted files=`["report.json"]`; active_retrieval_backend=`weaviate_hybrid`; active_retrieval_service_boundary=`weaviate`; collection=`SourceAtomNonprodRouteSelectedV2`; route_mode=`route_selected`; rollback_key=`weaviate_full_index_nonprod_rollback`; generator provider=`selected-evidence-deterministic-v1`; citation_format=`markdown-portfolio`.
- Formatter measurement: selected_evidence_citation_formatter_invoked=true; variants_available=`["compact","evidence-id","markdown-portfolio","source-locator"]`; formatted citations are stored in `answer_composer.formatted_citations`; retrieved_context_only_citations_diagnostic_only=true; portfolio_experiment_sidecar_written=false.
- Evidence gate enforce measurement: sufficient=`3`; insufficient=`3`; allowed_answer_count=`3`; abstained_count=`3`; unsupported_answer_blocked_count=`3`; unsupported_answer_rate_before_gate=`0.5`; unsupported_answer_rate_after_gate=`0.0`; citation_supported_count=`8`; citation_retrieved_context_only_diagnostic_count=`0`; citation_wrong_target_count=`0`; citation_missing_target_count=`0`; citation_unsupported_text_count=`0`.
- Before/after diagnostic delta against the prior extractive-v1 evidence-gate baseline: unsupported_answer_rate_after_gate stays `0.0`; unsupported_answer_blocked_count stays `3`; retrieved-context-only citation count changes `42 -> 0`; supported citation count remains narrowed to selected evidence (`8`) rather than broad retrieved-context citations.
- Residual row taxonomy: allowed rows are `text_namu_v2_0005`, `text_namu_v2_0017`, and `text_namu_v2_0077`; blocked rows are `text_namu_v2_0012`, `text_namu_v2_0014`, and `text_namu_v2_0084`, all with evidence-gate `missing_query_anchor`.
<!-- actual_rag_eval_selected_evidence_citation_formatter_checkpoint_c_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_selected_evidence_composer_checkpoint_b_nonprod:measurements-entry:start -->
### actual_rag_eval_selected_evidence_composer_checkpoint_b_nonprod

- Scope: non-production portfolio experiment Checkpoint A/B only. The active retrieval boundary stays Weaviate route-selected over SourceAtom/EvidenceBundle; full-index Weaviate rollback remains available through `ai/eval/configs/weaviate_full_index_nonprod_rollback.json`; official metrics, production readiness, gold/qrels/labels/answerability/expected/denominator/source-registry/current mutation, broad planner, Memory Agent, Tool Agent, and raw prompt/response artifacts remain closed.
- Fresh report: `reports/rag_eval/actual_rag_eval_selected_evidence_composer_checkpoint_b_nonprod_20260612_v4/report.json`; emitted files=`["report.json"]`; active_retrieval_backend=`weaviate_hybrid`; active_retrieval_service_boundary=`weaviate`; collection=`SourceAtomNonprodRouteSelectedV2`; route_mode=`route_selected`; rollback_key=`weaviate_full_index_nonprod_rollback`; generator provider=`selected-evidence-deterministic-v1`.
- Composer measurement: selected_evidence_composer_invoked=true; item_count=`6`; composer_abstained_count=`0`; selected_evidence_composer_selected_evidence_count=`8`; input_policy=`query_text_and_selected_sourceatom_evidence_only_no_gold_qrels_labels_ids_or_baseline`; extractive_v1_baseline_preserved_for_comparison=true.
- Evidence gate enforce measurement: sufficient=`3`; insufficient=`3`; allowed_answer_count=`3`; abstained_count=`3`; unsupported_answer_blocked_count=`3`; unsupported_answer_rate_before_gate=`0.5`; unsupported_answer_rate_after_gate=`0.0`; citation_supported_count=`8`; citation_retrieved_context_only_diagnostic_count=`0`; citation_wrong_target_count=`0`; citation_missing_target_count=`0`; citation_unsupported_text_count=`0`.
- Before/after diagnostic delta against the prior extractive-v1 evidence-gate baseline: unsupported_answer_rate_after_gate stays `0.0`; unsupported_answer_blocked_count stays `3`; retrieved-context-only citation count changes `42 -> 0`; supported citation count changes `18 -> 8` because final citations are narrowed to answer-supporting selected evidence rather than broad retrieved context.
- Residual row taxonomy: allowed rows are `text_namu_v2_0005`, `text_namu_v2_0017`, and `text_namu_v2_0077`; blocked rows are `text_namu_v2_0012`, `text_namu_v2_0014`, and `text_namu_v2_0084`, all with evidence-gate `missing_query_anchor`.
<!-- actual_rag_eval_selected_evidence_composer_checkpoint_b_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_weaviate_source_atom_boundary_nonprod:measurements-entry:start -->
### actual_rag_eval_weaviate_source_atom_boundary_nonprod

- Scope: SourceAtom/EvidenceBundle actual-RAG Weaviate service-boundary measurement only. Weaviate is selected over Chroma; local Python source-native layered retrieval, local corpus scans, diagnostic hash vectors, FAISS active retrieval, and SearchUnit/SearchView candidate surfaces are forbidden in the Weaviate lane.
- Preserved v1 baseline: collection=`SourceAtomNonprod`; namespace=`actual_rag_eval_nonprod`; schema_version=`weaviate_source_atom_v1`; streaming manifest=`reports/rag_eval/weaviate_source_atom_index_manifest_nonprod_streaming_full/index_manifest.json`; indexed_count=`136280`; embedding_model=`BAAI/bge-m3`; embedding_dim=`1024`; embedding_device=`cuda:0`; diagnostic_hash_vector_used=false; faiss_used_for_index_seed=false; faiss_used_for_active_retrieval=false; production_namespace=false.
- Fresh v1 full-index report: `reports/rag_eval/actual_rag_eval_text_gold_weaviate_hybrid_streaming_bge_m3_nonprod_20260612/report.json`; quality-gate report: `reports/rag_eval/actual_rag_eval_text_gold_weaviate_hybrid_streaming_bge_m3_nonprod_20260612/legacy_real_rag_quality_gate_report.json`; item JSONL: `reports/rag_eval/actual_rag_eval_text_gold_weaviate_hybrid_streaming_bge_m3_nonprod_20260612/legacy_real_rag_quality_gate_items.jsonl`.
- Active proof: active_retrieval_backend=`weaviate_hybrid`; active_retrieval_service_boundary=`weaviate`; external_vector_db.configured=true; external_vector_db.invoked=true; external_vector_db.backend=`weaviate`; external_vector_db.production_namespace=false; python_local_corpus_scan_used_for_candidate_generation=false; source_native_layered_retrieval_used_for_candidate_generation=false; diagnostic_hash_vector_used=false; faiss_used_for_active_retrieval=false; searchunit_searchview_used_as_candidate_surface=false; candidate_generation_input_policy=`query_text_and_query_embedding_only_no_gold_qrels_labels_ids_or_baseline`.
- V1 retrieval metrics: item_count=`6`; retrieval_empty_rate=`0.0`; vector_candidate_count_avg=`10.0`; bm25_candidate_count_avg=`10.0`; hybrid_candidate_count_avg=`10.0`; weak_evidence_match_recall@10=`0.666667`; judged_answer_correctness_provisional=`0.666667`; e2e_rag_success_provisional=`0.666667`; target_span_present_but_not_retrieved_count=`0`; hybrid latency p50/p95=`24.18295/28.4395` ms; evidence package sufficient/insufficient=`4/2`; guardrails valid with violations=`[]`.
- Route-selected default config measurement: `ai/eval/configs/weaviate_route_selected_nonprod_default.json`; routine report=`reports/rag_eval/actual_rag_eval_weaviate_route_selected_nonprod_default_report_only_20260612_v2/report.json`; active_retrieval_backend=`weaviate_hybrid`; active_retrieval_service_boundary=`weaviate`; collection=`SourceAtomNonprodRouteSelectedV2`; schema_version_source_atom=`weaviate_source_atom_v2`; route_planner_version=`weaviate_route_planner_v1`; fallback_used=false; fail_closed_on_unavailable=true; emitted files=`["report.json"]`; promotion_decision=`blocked_keep_full_index_rollback`; promotion_blockers=`["route_ab_comparative_gate_not_run_in_routine_single_mode"]`; rollback_key=`weaviate_full_index_nonprod_rollback`.
- Route-selected default filters sent to Weaviate: `source_family=TEXT`; `granularity=["paragraph","heading_context_block","caption"]`; `retrieval_route=text_general`; base filters include namespace=`actual_rag_eval_nonprod` and visibility=`nonprod`; route_filter_sent=true; weaviate_filter_sent=true; metadata_filters_sent_to_weaviate=true; python_post_filtering=`safety_validation_only`.
- Full-index rollback config measurement: `ai/eval/configs/weaviate_full_index_nonprod_rollback.json`; rollback_key=`weaviate_full_index_nonprod_rollback`; collection=`SourceAtomNonprod`; schema_version_source_atom=`weaviate_source_atom_v1`; route_mode=`full_index`; fallback_used=false; fail_closed_on_unavailable=true; this path is the Weaviate rollback baseline, not a local fallback.
- Route-selected v2 index: collection=`SourceAtomNonprodRouteSelectedV2`; schema_version_source_atom=`weaviate_source_atom_v2`; manifest=`reports/rag_eval/weaviate_source_atom_index_manifest_nonprod_route_selected_v2/index_manifest.json`; checkpoint=`reports/rag_eval/weaviate_source_atom_index_manifest_nonprod_route_selected_v2/index_checkpoint.json`; index_object_count=`136280`; vectorized_object_count=`136184`; metadata_only_object_count=`96`; vectorized_object_ratio=`0.999296`; vectorized_by_granularity=`{"heading_context_block":8,"paragraph":135927,"table_row":249}`; metadata_only_by_granularity=`{"cell":96}`; metadata_only_by_source_family=`{"XLSX":96}`; schema_index_v2_rebuild_required_for_metadata_only_policy=false.
- Route-selected A/B report: `reports/rag_eval/actual_rag_eval_weaviate_route_selected_nonprod_default_ab_20260612_v2/route_selected_hybrid_evidence_store_ab_report.json`; item sidecar: `reports/rag_eval/actual_rag_eval_weaviate_route_selected_nonprod_default_ab_20260612_v2/route_selected_hybrid_evidence_store_ab_items.jsonl`; primary report: `reports/rag_eval/actual_rag_eval_weaviate_route_selected_nonprod_default_ab_20260612_v2/report.json`; recommendation=`promote_route_selected_nonprod_default`; promotion_blockers=`[]`; primary report mirrors promotion_decision, blockers, rollback_key, residual_risks, and next_recommended_goal.
- Text-regression A/B lanes (`6` rows): Lane A full-index collection=`SourceAtomNonprod`, schema=`weaviate_source_atom_v1`, weak@10=`0.666667`, duplicate/same-doc=`0.066667/0.266667`, route_filter_sent=false; Lane B TEXT-only weak@10=`0.666667`, text_route_degradation_count=`0`, text_route_degradation_ok=true; Lane C mixed-pool weak@10=`0.666667`, pollution=`0`; Lane D route-selected collection=`SourceAtomNonprodRouteSelectedV2`, schema=`weaviate_source_atom_v2`, weak@10=`0.666667`, wrong_source_family=`0`, duplicate/same-doc=`0.0/0.163265`, route_filter_sent=true.
- Mixed-route diagnostic (`29` rows from v5_5 packet): Lane C pollution=`127`, weak@10=`0.413793`, duplicate/same-doc=`0.017241/0.293103`, source_family_distribution=`{"PDF":6,"TEXT":204,"XLSX":80}`; Lane D wrong_source_family=`0`, weak@10=`0.413793`, duplicate/same-doc=`0.0/0.151724`, source_family_distribution=`{"PDF":20,"TEXT":66,"XLSX":59}`, p50/p95=`9.4817/20.9013` ms. Mixed diagnostic weak@10 delta=`0.0`; allowed drop=`0.034483`; duplicate result delta=`-0.017241`; same-doc duplicate delta=`-0.141379`; mixed_route_diagnostic_quality_ok=true.
- Guardrail status: all lanes record active boundary=`weaviate`, external_vector_db.invoked=true, local corpus scan=false, source-native layered retrieval=false, diagnostic hash=false, FAISS active retrieval=false, SearchUnit/SearchView candidate surface=false, raw prompt/response artifacts=false, and gold_mutation=false. Lane B and Lane D record source_family/granularity filters sent to Weaviate; Python post-filtering is safety validation only.
- Next recommended goal: `selected_evidence_answer_composer_citation_formatter_nonprod`; retrieved-context-only citations stay diagnostic-only, and broad planner, Memory Agent, Tool Agent, gold expansion, and readiness gates remain closed.
<!-- actual_rag_eval_weaviate_source_atom_boundary_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_source_native_evidence_gate_nonprod:measurements-entry:start -->
### actual_rag_eval_source_native_evidence_gate_nonprod

- Scope: bounded evidence/citation validator plus abstention gate for the legacy-free source-native real-RAG lane. It reuses the existing six-row text gold set and frozen legacy baseline only for post-run diagnostics; enforcement uses no expected answer/evidence, qrels, labels, legacy output, row ID, or target ID.
- Diagnostic run: `reports/rag_eval/actual_rag_eval_text_gold_evidence_gate_diagnostic_nonprod_20260611_v4/report.json`; quality-gate report: `reports/rag_eval/actual_rag_eval_text_gold_evidence_gate_diagnostic_nonprod_20260611_v4/legacy_real_rag_quality_gate_report.json`; item JSONL: `reports/rag_eval/actual_rag_eval_text_gold_evidence_gate_diagnostic_nonprod_20260611_v4/legacy_real_rag_quality_gate_items.jsonl`.
- Enforce run: `reports/rag_eval/actual_rag_eval_text_gold_evidence_gate_enforce_nonprod_20260611_v3/report.json`; quality-gate report: `reports/rag_eval/actual_rag_eval_text_gold_evidence_gate_enforce_nonprod_20260611_v3/legacy_real_rag_quality_gate_report.json`; item JSONL: `reports/rag_eval/actual_rag_eval_text_gold_evidence_gate_enforce_nonprod_20260611_v3/legacy_real_rag_quality_gate_items.jsonl`.
- Validator: version=`bounded_evidence_gate_v1`; selected_surface=`source_native`; selected_backend=`hybrid`; embedding_model=`BAAI/bge-m3`; embedding_device=`cuda:0`; source_native_unit_count=`136280`; SearchUnit/SearchView candidate surface=false; retrieval_loop_triggered=false.
- Diagnostic gate summary: item_count=`6`; sufficient=`3`; insufficient=`3`; conflicting=`0`; unresolved=`0`; allowed=`3`; abstained=`0`; unsupported_answer_blocked=`0`; would_abstain=`3`; would_block_unsupported_answer=`3`; unsupported_answer_rate_before_gate=`0.5`; unsupported_answer_rate_after_gate=`0.5`.
- Enforce gate summary: item_count=`6`; sufficient=`3`; insufficient=`3`; conflicting=`0`; unresolved=`0`; allowed=`3`; abstained=`3`; unsupported_answer_blocked=`3`; insufficient_evidence_abstained=`3`; sufficient_evidence_allowed=`3`; sufficient_evidence_over_abstain=`0`; unsupported_answer_rate_before_gate=`0.5`; unsupported_answer_rate_after_gate=`0.0`.
- Item decisions after enforce: allowed `text_namu_v2_0005`, `text_namu_v2_0017`, `text_namu_v2_0077`; abstained/blocked `text_namu_v2_0012` missing query anchors `나이`, `생일`; `text_namu_v2_0014` missing `구십칠`, `애드버서리`; `text_namu_v2_0084` missing `소드아트`, `오디널`.
- Citation support against selected evidence: supported=`18`; retrieved_context_only_diagnostic=`42`; wrong_target=`0`; missing_target=`0`; unsupported_text=`0`. `citation_points_to_retrieved_context_rate` remains diagnostic-only and is not strict citation correctness.
- Guardrails: valid=true; expected_gold_legacy_not_used_for_evidence_gate_enforcement=true; expected_fields_not_used_for_candidate_generation=true; legacy_outputs_not_used_for_candidate_generation=true; gold_qrels_labels_not_mutated=true; searchunit_searchview_not_used_in_real_rag_lane=true; source_native_selected=true; source_native_units_only=true; raw_prompt_response_payloads_not_written=true; production_namespace_untouched=true; official_metric_closed=true.
<!-- actual_rag_eval_source_native_evidence_gate_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_legacy_real_rag_quality_gate_nonprod:measurements-entry:start -->
### actual_rag_eval_legacy_real_rag_quality_gate_nonprod

- Scope: legacy-free real-RAG parity and evidence-package quality gate only. It reuses the existing text gold set, replays a frozen SearchUnit/SearchView baseline artifact after the fresh run, and does not create, expand, review, or mutate gold/qrels/labels.
- Fresh real-RAG report: `reports/rag_eval/actual_rag_eval_text_gold_legacy_real_rag_quality_gate_nonprod_20260611/report.json`; quality-gate summary: `reports/rag_eval/actual_rag_eval_text_gold_legacy_real_rag_quality_gate_nonprod_20260611/legacy_real_rag_quality_gate_report.json`; item report: `reports/rag_eval/actual_rag_eval_text_gold_legacy_real_rag_quality_gate_nonprod_20260611/legacy_real_rag_quality_gate_items.jsonl`.
- Existing gold set: `ai/eval/eval_queries/gold_queries_text_namu_v2_1_question_gold_v2.csv`; item_count=`6`; schema_version unavailable; selection rationale=`existing actual-RAG text-gold dataset reused from requested/latest actual-RAG coverage`; gold_mutation=false.
- Frozen baseline: `reports/rag_eval/actual_rag_eval_text_gold_single_vector_final_20260611/report.json`; run_id=`actual_rag_eval_text_gold_single_vector_final_20260611`; item_count=`6`; legacy_baseline_replayed_not_executed=true.
- Real lane: selected_surface=`source_native`; selected_backend=`hybrid`; embedding_model=`BAAI/bge-m3`; embedding_device=`cuda:0`; vector_dim=`1024`; source_native_unit_count=`136280`; source_native_units_only=true; SearchUnit/SearchView candidate surface=false; candidate_generation_input_policy=`query_text_only`.
- Answer parity: comparable=`6/6`; exact_same=`0`; normalized_same=`0`; equivalent=`1`; same_answer_same_support=`0`; same_answer_different_support=`0`; legacy_correct_real_wrong=`0`; legacy_wrong_real_correct=`3`; both_correct=`1`; both_wrong=`2`; not_comparable=`0`.
- Evidence/citation support: evidence_package_status sufficient=`3`, insufficient=`3`; real_rag_supported=`3`; legacy_supported=`0`; unsupported_same_answer=`1`; answer_delta_category_counts=`{"both_correct_different_wording":1,"both_wrong_different":2,"legacy_wrong_real_correct":3}`.
- Diagnostic critic: critic_result_tier=`diagnostic`; answer_supported_by_evidence=`3`; citation_supported_by_evidence=`3`; citation_points_to_expected_or_resolved_evidence=`3`; citation_points_to_retrieved_context_diagnostic_only=`6`; evidence_sufficient=`3`; needs_more_retrieval=`3`; should_abstain=`3`; retrieval_loop_triggered=false.
- Guardrails: valid=true; gold_qrels_labels_not_mutated=true; expected_fields_not_used_for_candidate_generation=true; legacy_outputs_not_used_for_candidate_generation=true; searchunit_searchview_not_used_in_real_rag_lane=true; source_native_selected=true; source_native_units_only=true; production_namespace_untouched=true; raw_prompt_response_payloads_not_written=true; official_metric_closed=true; violations=`[]`.
<!-- actual_rag_eval_legacy_real_rag_quality_gate_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_source_native_evidence_resolution_loop_nonprod:measurements-entry:start -->
### actual_rag_eval_source_native_evidence_resolution_loop_nonprod

- Scope: source-native evidence-resolution review loop closure only. The routine surface remains SourceAtom/EvidenceBundle-backed `source_native`; SearchUnit/SearchView remains `legacy_comparison_debug_only`; generation remains `extractive-v1` diagnostic smoke.
- Fresh routine report: `reports/rag_eval/actual_rag_eval_text_gold_source_native_evidence_loop_routine_nonprod_20260611_v2/report.json`; generated_at=`2026-06-11T10:52:59Z`; directory files=`report.json` only; output_mode=`single`; human_review_packet.enabled=false; reviewed_mapping_applied=false; reviewed_mapping_row_count=`0`.
- Explicit review packet report: `reports/rag_eval/actual_rag_eval_text_gold_source_native_evidence_loop_review_packet_nonprod_20260611_v2/report.json`; CSV=`reports/rag_eval/actual_rag_eval_text_gold_source_native_evidence_loop_review_packet_nonprod_20260611_v2/human_review_packet.csv`; row_count=`30`; all `human_*` fields blank; machine recommendations are not gold.
- Reviewed mapping ingest: CLI/API flag is `--reviewed-evidence-mapping-csv`; accepted fixture ingest creates `reviewed_evidence_mapping_patch.json`, applies a run-local overlay, records `reviewed_mapping_input_path`, `reviewed_mapping_applied`, `reviewed_mapping_row_count`, and `denominator_changes`, rejects blank human-decision rows, and rejects machine recommendations as human decisions.
- Metric before/after against `actual_rag_eval_text_gold_source_native_phase0_phase2_full_corpus_bge_m3_nonprod_20260611_v2`: strict answer `0/0 -> 0/0`; strict evidence@10 `0/0 -> 0/0`; strict citation_precision `6/60 -> 0/0`; strict citation_recall `3/6 -> 0/0`; provisional judged_answer_correctness `3/6 -> 3/6`; weak_evidence_match_recall@10 `3/6 -> 3/6`; resolved_evidence_available_rate `3/6 -> 3/6`; e2e_rag_success_provisional `3/6 -> 3/6`.
- Full-corpus review-only resolver: scope=`full-corpus-review-only`; candidate_count=`30`; high/medium/low=`13/15/2`; review_only_count=`30`; resolved_candidate_count=`3`; collision_count=`12`; unresolved_count=`3`; gold_or_qrels_mutation=false; human_decision_fields_filled_by_codex=false.
- Denominator changes in the real six-row routine run: strict_answer_denominator `0 -> 0`; strict_evidence_denominator `0 -> 0`; strict_e2e_denominator `0 -> 0`; expected_evidence_row_count `6 -> 6`; unknown_answerability_count `6 -> 6`.
- Canonical diagnostic failure counts: gold_missing_answerability=`6`, schema_warning=`6`, metric_not_applicable=`6`, citation_wrong=`6`, retrieved_not_validated=`6`, answer_judge_fail=`3`, expected_evidence_id_unresolved=`3`, present_not_retrieved=`2`.
<!-- actual_rag_eval_source_native_evidence_resolution_loop_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_source_native_phase0_phase2_full_corpus_bge_m3_nonprod:measurements-entry:start -->
### actual_rag_eval_source_native_phase0_phase2_full_corpus_bge_m3_nonprod

- Scope: Phase 0-2 source-native actual-RAG contract stabilization only. Routine surface is SourceAtom/EvidenceBundle-backed `source_native`; SearchUnit/SearchView remains `legacy_comparison_debug_only`; `extractive-v1`, answer/context consistency, citation/context consistency, source-native retrieval diagnostics, MMR diagnostics, and resolver candidates are diagnostic-only.
- Fresh report: `reports/rag_eval/actual_rag_eval_text_gold_source_native_phase0_phase2_full_corpus_bge_m3_nonprod_20260611_v2/report.json`; generated_at=`2026-06-11T10:02:04Z`; directory files=`report.json` only; output_mode=`single`; human_review_packet.enabled=false; human_review_packet.row_count=0.
- Vector path: selected_backend=`hybrid`; embedding_model=`BAAI/bge-m3`; embedding_device=`cuda:0`; gpu_used_for_embedding=true; vector_index_kind=`faiss`; vector_index_type=`IndexFlatIP`; vector_dim=`1024`; faiss_index_ntotal=`136280`; corpus_fingerprint_sha256=`sha256:a000bf4ba823a68ccd5182303daf7960e610692af4a2251318ca49e755a4920b`; text_hash_count=`136280`; bge_m3_replacement_needed=false; semantic_quality_claim_allowed=false.
- Metric before/after against `actual_rag_eval_text_gold_source_native_bge_m3_mmr_final_20260611`: strict answer denominator `0 -> 0`; strict evidence@10 denominator `0 -> 0`; provisional judged_answer_correctness `3/6 -> 3/6`; weak_evidence_match_recall@10 `3/6 -> 3/6`; e2e_rag_success_provisional `3/6 -> 3/6`.
- Full-corpus expected-evidence resolver: scope=`full-corpus`; candidate_count=`19`; high/medium/low=`11/7/1`; review_only_count=`19`; resolved_candidate_count=`2`; collision_count=`6`; unresolved_count=`4`; gold_or_qrels_mutation=false; human_decision_fields_filled_by_codex=false. Expected evidence remained review-only and was not used for retrieval candidate generation.
- Canonical diagnostic failure counts: gold_missing_answerability=`6`, schema_warning=`6`, metric_not_applicable=`6`, citation_wrong=`6`, retrieved_not_validated=`6`, answer_judge_fail=`3`, expected_evidence_id_unresolved=`3`, present_not_retrieved=`2`.
<!-- actual_rag_eval_source_native_phase0_phase2_full_corpus_bge_m3_nonprod:measurements-entry:end -->
...
<!-- actual_rag_eval_source_native_bge_m3_mmr_nonprod:measurements-entry:start -->
### actual_rag_eval_source_native_bge_m3_mmr_nonprod

- Scope: non-production actual-RAG source-native BGE-M3 index replacement plus post-retrieval source-native retrieval diagnostics. Candidate generation remains query-only over source-native SourceAtom/EvidenceBundle-owned units; expected/gold/qrels/labels/answerability/IDs/baseline top-k are used only after retrieval for diagnostic scoring. MMR is reported as `mmr_selected` selection diagnostics, not MRR/reciprocal-rank and not an official metric.
- BGE-M3 index build: `ai/eval/indexes/rag-data-all-source-citable-nonprod-bge-m3-v1/build.json`; embedding_model=`BAAI/bge-m3`; embedding_device=`cuda:0`; gpu_used_for_embedding=true; dimension=`1024`; chunk_count=`136280`; faiss_index_ntotal=`136280`; faiss_index_type=`IndexFlatIP`; faiss_row_id_mismatch_count=`0`; embedding_build_latency_ms=`1841409.9305`; index_build_latency_ms=`110.8482`; diagnostic_only=true; semantic_quality_claim_allowed=false; gold_fields_used_for_index_build=false; expected_fields_used_for_index_build=false; qrels_used_for_index_build=false; labels_used_for_index_build=false. Persisted files are `build.json`, `faiss.index`, and `search_view_manifest.jsonl`.
- Fixture BGE-M3 report: `reports/rag_eval/actual_rag_eval_fixture_source_native_bge_m3_mmr_final_20260611/report.json`; directory files=`report.json` only; selected_surface=`source_native`; selected_backend=`hybrid`; embedding_model=`BAAI/bge-m3`; embedding_device=`cuda:0`; source_native_unit_count=`136280`; strict evidence_recall@10=`1.0`; provisional judged_answer_correctness=`1.0`; selected hit@1/3/5/10=`0.0/1.0/1.0/1.0`; vector/hybrid hit@1/3/5/10=`1.0/1.0/1.0/1.0`; `mmr_selected` hit@10=`0.0` and nDCG@10=`0.0`, marking MMR selection as a repair target rather than a win.
- Text-golden BGE-M3 heuristic report: `reports/rag_eval/actual_rag_eval_text_gold_source_native_bge_m3_mmr_final_20260611/report.json`; directory files=`report.json` only; selected_surface=`source_native`; selected_backend=`hybrid`; embedding_model=`BAAI/bge-m3`; embedding_device=`cuda:0`; vector_dim=`1024`; source_native_unit_count=`136280`; `semantic_quality_samples.sample_count=6`; raw_prompt_payload_written=false; raw_response_payload_written=false. Compared to diagnostic hash run `actual_rag_eval_text_gold_source_native_vector_audit_final_20260611_v2`: judged_answer_correctness_provisional=`0.333333 -> 0.5`; weak_evidence_match_recall@10=`0.333333 -> 0.5`; e2e_rag_success_provisional=`0.333333 -> 0.5`; citation_precision=`0.066667 -> 0.1`; citation_recall=`0.333333 -> 0.5`; source_native_present_not_retrieved_count=`3 -> 2`. These are diagnostic deltas only.
- Text-golden source-native retrieval diagnostics: selected denominator=`6`, hit@1/3/5/10=`0.166667/0.333333/0.333333/0.5`, nDCG@10=`0.327377`; vector hit@1/3/5/10=`0.5/0.5/0.5/0.666667`, nDCG@10=`0.559368`; hybrid hit@1/3/5/10=`0.333333/0.5/0.666667/0.666667`, nDCG@10=`0.502964`; BM25 hit@1/3/5/10=`0.166667/0.166667/0.166667/0.333333`, nDCG@10=`0.219244`; `mmr_selected` hit@1/3/5/10=`0.0/0.0/0.166667/0.333333`, nDCG@10=`0.123843`, mmr_lambda=`0.65`, candidate_pool_k_max=`26`.
- Text-golden BGE-M3 local-LLM judge report: `reports/rag_eval/actual_rag_eval_text_gold_source_native_bge_m3_mmr_llmjudge_final_20260611/report.json`; directory files=`report.json` only; judge_kind=`local_llm_strict_json`; backend=`llamacpp`; model=`gemma4-e2b-local`; base_url=`http://127.0.0.1:8081`; available=true; threshold=`0.6`; judged_answer_correctness_provisional=`0.5`; weak_evidence_match_recall@10=`0.5`; e2e_rag_success_provisional=`0.5`. Semantic samples contain actual query, current generated answer excerpt, bounded retrieved context previews, LLM judge reason/confidence, and raw_response_sha256. They do not write raw prompt or raw response artifacts.
- Guardrails and bottlenecks: official_metric=false; official_metric_input_rows=0; protected_namespaces_touched=[]; SearchUnit/SearchView candidate surface disabled in routine auto and preserved only as `legacy_comparison_debug_only`; current alias, source registry truth, gold/qrels/labels/answerability/expected fields, strict denominator, production routing, product-success, promotion, and live-readiness gates stayed closed. Remaining bottlenecks are source-native ranking/query formulation, poor MMR selection behavior on this denominator, and `extractive-v1` answer generation before any LLM-generation quality claim.
<!-- actual_rag_eval_source_native_bge_m3_mmr_nonprod:measurements-entry:end -->

<!-- actual_rag_eval_source_native_vector_index_audit_nonprod:measurements-entry:start -->
### actual_rag_eval_source_native_vector_index_audit_nonprod

- Scope: non-production actual-RAG source-native vector index audit and final-target clarification. Routine retrieval target is source-native RAG over SourceAtom/EvidenceBundle evidence truth; SearchUnit/SearchView remains `legacy_comparison_debug_only`; vector retrieval is a semantic/paraphrase candidate generator, not an oracle, not evidence truth, and not a semantic-quality claim. Candidate generation uses query text, source-native unit text/metadata, backend scores, FAISS/id-map hydration, and layer provenance only. Expected/gold/qrels/evidence IDs/baseline top-k are closed to candidate generation and used only for post-retrieval diagnostics where explicitly labeled.
- Fixture report: `reports/rag_eval/actual_rag_eval_fixture_source_native_vector_audit_final_20260611_v2/report.json`; directory files=`report.json` only; selected_surface=`source_native`; selected_backend=`hybrid`; vector_audit.status=`connected_semantic_quality_unproven`; vector_backend=`python_deterministic_test`; embedding_model=`deterministic-test-source-native-vector`; embedding_dim=`16`; embedding_device=`cpu_python_test_adapter`; faiss_index_ntotal/id_map/source_native_unit_count=`1/1/1`; index_integrity_passed=true; query_invocation_passed=true; hydration_passed=true; external VectorDB configured/invoked/reachable all false; semantic_quality_claim_allowed=false; bge_m3_replacement_needed=true.
- Text-golden report: `reports/rag_eval/actual_rag_eval_text_gold_source_native_vector_audit_final_20260611_v2/report.json`; directory files=`report.json` only; selected_surface=`source_native`; selected_backend=`hybrid`; vector_audit.status=`connected_semantic_quality_unproven`; vector_backend=`faiss`; embedding_model=`codex-diagnostic-hashing-vector-v1`; embedding_dim=`128`; embedding_device=`cpu_existing_nonprod_index`; gpu_used_for_embedding=false; build_index_version=`official-answer-citation-agentic-loop-v3-7-1-all-source-citable-nonprod-v1`; build_chunk_count=`136280`; faiss_index_ntotal/id_map/source_native_unit_count=`136280/136280/136280`; faiss_row_id_available_count=`136280`; faiss_row_id_mismatch_count=`0`; index_integrity_passed=true; semantic_quality_claim_allowed=false; bge_m3_replacement_needed=true.
- Text-golden invocation/hydration: vector_backend_invoked_count=`6`; query_embedding_created_or_loaded_count=`6`; vector_candidate_count=`60`; vector_hydration_success_count=`60`; vector_hydration_failure_count=`0`; query_invocation_passed=true; hydration_passed=true. External VectorDB configured/invoked/reachable all false, so the current routine path is local source-native FAISS/id-map hydration, not an external vector database.
- Text-golden BM25/vector/hybrid comparison: candidate_count_avg=`10.0/10.0/10.0`; retrieval_empty_rate=`0.0/0.0/0.0`; bm25_vector_topk_overlap_avg=`0.0`; bm25_only_candidate_count_avg=`10.0`; vector_only_candidate_count_avg=`10.0`; hybrid_contains_vector_only_candidate_count_avg=`5.0`; hybrid_contains_bm25_only_candidate_count_avg=`5.0`; vector_contribution_to_hybrid_topk_count_avg=`5.0`; bm25_contribution_to_hybrid_topk_count_avg=`5.0`; vector_contribution_to_selected_topk_count_avg=`0.0`; bm25_contribution_to_selected_topk_count_avg=`9.5`; merge_policy=`source_native_layered_rrf_bm25_vector_hybrid_diagnostic`.
- Post-retrieval diagnostics only: BM25/vector/hybrid expected-anchor recall@k=`0.333333/0.166667/0.333333`; target-present-not-retrieved count=`4/5/4`; expected/gold/qrels/IDs/baseline candidate-generation flags are all false; expected_fields_used_for_post_retrieval_diagnostics=true. Remaining bottleneck: the vector path is wired and hydrated but still uses diagnostic hash embeddings and contributes no final selected top-k evidence on the six-row text-golden run. GPU BGE-M3 source-native index replacement and source-native ranking remeasurement are future non-cleanup repair lanes before any broader quality/product-readiness claim.
<!-- actual_rag_eval_source_native_vector_index_audit_nonprod:measurements-entry:end -->
...
- Retrieval backend for fixture/text-golden: requested `auto`, selected `hybrid`; BM25 enabled `true`, vector enabled `true`, hybrid enabled `true`; embedding model `BAAI/bge-m3`; embedding device `cuda:0`; gpu_used_for_embedding `true`; vector index kind `faiss`; vector index type `IndexFlatIP`; vector_dim `1024`; indexed_unit_count `300`. Superseded historical measurement: this proves single-artifact output and local FAISS wiring for a legacy SearchUnit/SearchView adapter, not the current routine source-native surface or semantic quality.
