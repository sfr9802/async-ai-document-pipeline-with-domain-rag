...
- Route-selected A/B report: `reports/rag_eval/actual_rag_eval_weaviate_route_selected_hybrid_store_ab_nonprod_route_v2_cellcap_sanitized_20260612/route_selected_hybrid_evidence_store_ab_report.json`; item sidecar: `reports/rag_eval/actual_rag_eval_weaviate_route_selected_hybrid_store_ab_nonprod_route_v2_cellcap_sanitized_20260612/route_selected_hybrid_evidence_store_ab_items.jsonl`; primary report: `reports/rag_eval/actual_rag_eval_weaviate_route_selected_hybrid_store_ab_nonprod_route_v2_cellcap_sanitized_20260612/report.json`; recommendation=`promote_route_selected_nonprod_default`; promotion_blockers=`[]`.
- Text-regression A/B lanes (`6` rows): Lane A full-index weak@10=`0.666667`, sufficient/insufficient=`4/2`, duplicate/same-doc=`0.066667/0.266667`, p50/p95=`13.1242/14.4287` ms; Lane B TEXT-only weak@10=`0.666667`, text_route_degradation_count=`0`, text_route_degradation_ok=true, p50/p95=`11.95915/14.663` ms; Lane C mixed-pool weak@10=`0.666667`, pollution=`0`, p50/p95=`12.6459/13.4153` ms; Lane D route-selected weak@10=`0.666667`, wrong_source_family=`0`, duplicate/same-doc=`0.0/0.163265`, p50/p95=`12.3368/13.8328` ms.
- Mixed-route diagnostic (`29` rows from v5_5 packet): Lane C pollution=`127`, weak@10=`0.413793`, duplicate/same-doc=`0.017241/0.293103`, source_family_distribution=`{"PDF":6,"TEXT":204,"XLSX":80}`, granularity_distribution=`{"cell":17,"paragraph":208,"table_row":65}`; Lane D wrong_source_family=`0`, weak@10=`0.413793`, duplicate/same-doc=`0.0/0.151724`, source_family_distribution=`{"PDF":20,"TEXT":66,"XLSX":59}`, granularity_distribution=`{"cell":17,"paragraph":85,"table_row":43}`, p50/p95=`9.4975/18.4814` ms. Mixed diagnostic weak@10 delta=`0.0`; allowed drop=`0.034483`; same-doc duplicate delta=`-0.141379`; route-selected duplicate collapse removed `145` same-doc candidates; bounded neighbor expansion issued `15` Weaviate-ID-only queries and added `0` neighbors.
