<!-- actual_rag_eval_source_native_surface_nonprod:triage-entry:start -->
- actual_rag_eval_source_native_surface_nonprod: actual-RAG retrieval now treats SourceAtom/EvidenceBundle-backed source-native units as the default truth-nearest surface when available, with SearchUnit/SearchView demoted to a legacy baseline inside the same single `report.json`. Text-golden diagnostics show source-native contains normalized expected evidence for 6/6 rows while SearchUnit/SearchView contains target spans for 0/6; source-native retrieved matching evidence for 2/6 and missed 4/6 target-present rows. SearchUnit/SearchView therefore is not the next repair target. The top bottleneck is source-native ranking/query formulation over the source-owned corpus, followed by replacing the current diagnostic hash FAISS index with a GPU BGE-M3 source-native index when feasible. GPU preflight found CUDA available, but the selected existing non-production source-native FAISS index uses `codex-diagnostic-hashing-vector-v1` on CPU, and the fallback reason is recorded. Routine output remains exactly one `report.json`; no human review packet or evidence sidecar was written. No gold/qrels/labels/answerability/expected fields, strict denominator, current alias, official metric, product-success, promotion, production routing, or live-readiness surface changed.
<!-- actual_rag_eval_source_native_surface_nonprod:triage-entry:end -->
