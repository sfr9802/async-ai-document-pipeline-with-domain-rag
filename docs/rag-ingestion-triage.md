...
- actual_rag_eval_weaviate_source_atom_boundary_nonprod: the SourceAtom/EvidenceBundle retrieval lane proves a Weaviate service boundary in non-production while preserving the current full-index baseline. The v1 streaming index remains collection `SourceAtomNonprod`, namespace `actual_rag_eval_nonprod`, indexed_count=`136280`, self-provided BAAI/bge-m3 1024-d vectors, diagnostic_hash_vector_used=false, and faiss_used_for_index_seed=false. The fresh full-index run `actual_rag_eval_text_gold_weaviate_hybrid_streaming_bge_m3_nonprod_20260612` selected `source_native` plus `weaviate_hybrid`, proved external_vector_db invoked/reachable/backend=`true/true/weaviate`, production_namespace=false, local Python corpus scan=false, source-native layered retrieval=false, diagnostic hash vector=false, FAISS active retrieval=false, SearchUnit/SearchView candidate surface=false, and candidate_generation_input_policy=`query_text_and_query_embedding_only_no_gold_qrels_labels_ids_or_baseline`. Full-index Weaviate metrics remain weak evidence@10=`0.666667`, evidence packages sufficient/insufficient=`4/2`, and hybrid latency p50/p95=`24.18295/28.4395` ms. The fallback risk is guarded by runtime fail-closed behavior and tests.
- actual_rag_eval_weaviate_route_selected_hybrid_store_ab_nonprod: the route-selected A/B harness is implemented, rerun on the route-selected v2 non-production collection, and measured without replacing the default path. V2 index `SourceAtomNonprodRouteSelectedV2` materializes taxonomy, source-owned structural locator metadata, and metadata-only policy: index_object_count=`136280`, vectorized_object_count=`136184`, metadata_only_object_count=`96`, vectorized_object_ratio=`0.999296`, vectorized_by_granularity=`{"heading_context_block":8,"paragraph":135927,"table_row":249}`, and metadata_only_by_granularity=`{"cell":96}`. Report `reports/rag_eval/actual_rag_eval_weaviate_route_selected_hybrid_store_ab_nonprod_route_v2_cellcap_sanitized_20260612/route_selected_hybrid_evidence_store_ab_report.json` compares Lane A full-index, Lane B TEXT-only, Lane C mixed-pool, and Lane D route-selected over the existing 6-row text regression set, plus a 29-row mixed route diagnostic from the existing v5_5 packet. Guardrails are clean in every lane: Weaviate invoked=true; local corpus scan, source-native layered retrieval, diagnostic hash vectors, FAISS active retrieval, and SearchUnit/SearchView candidate surfaces are all false; gold/qrels/labels/expected/denominator/current/source-registry/human fields were not mutated; raw prompt/response payloads were not written. TEXT did not degrade: Lane A/B/D weak@10 all `0.666667`, text_route_degradation_count=`0`, text_route_degradation_ok=true. Mixed-route diagnostic confirms route pollution and route-selected recovery: Lane C mixed-pool pollution=`127`, while Lane D route-selected wrong_source_family=`0`; Lane D weak@10 stayed `0.413793` versus Lane C `0.413793`, duplicate rate fell `0.017241 -> 0.0`, and same-doc duplicate rate fell `0.293103 -> 0.151724` after bounded duplicate collapse. Latency stayed acceptable (primary Lane A p95=`14.4287` ms, Lane D p95=`13.8328` ms; mixed diagnostic Lane D p95=`18.4814` ms with bounded neighbor query count `15`). Recommendation is `promote_route_selected_nonprod_default`, with blockers `[]`. Next target: promote route-selected as an explicit non-production default/config path while preserving rollback to the full-index baseline; answer/citation composer work remains the next lane only if retrieval stays clean.
...
- DIAGNOSTIC_ARTIFACT_HOLD: ignored `ai/eval/reports/rag-ingestion/**`, report/status/archive manifests, v3-v7 run bundles, sidecars required by report artifact maps, and historical `quality/**` evidence. Do not bulk-delete; external archive plus hash verification is required before any future removal.
