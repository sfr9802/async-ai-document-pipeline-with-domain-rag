<!-- agentic_rag_xlsx_v2_query_anchor_rerun_repair_selection_20260629:triage-entry:start -->
- agentic_rag_xlsx_v2_query_anchor_rerun_repair_selection_20260629: Fresh deterministic report `reports/rag_eval/actual_rag_eval_gold29_xlsx_locator_query_anchor_diagnostic_rerun_v2_nonprod_20260629/report.json` and RunStore `reports/rag_eval/actual_rag_eval_gold29_xlsx_locator_query_anchor_diagnostic_rerun_v2_nonprod_20260629/run.sqlite` populated the compact `query_anchor_tool_acceptance_diagnostic` on live V2 output. It kept `official_metric=false`, `official_metric_input_rows=0`, projected the complete-manifest metric gate as ready/unblocked, left protected surfaces unchanged, and had no raw payload/secret scan matches.
- Deterministic bottleneck: the locator had `18` invocations, `72` candidates, `6` accepted, and `66` candidates rejected as `missing_query_anchor_after_tool`. Top missing anchors were dominated by generic Korean intent/question forms plus structural date/line/value anchors: `명입니까=39`, `지정된=27`, `무엇입니까=20`, `2018년=11`, `5월=11`, `2019년=9`, `3월=8`, `11월=7`, `언제입니까=7`. Source-owned fields were present on rejected candidates (`cell_range=66`, `source_date_aliases=47`, `display_value=38`, `row_label=38`, `target_column=38`), but all `66` query-anchor rejected candidates lacked candidate-level validated-axis evidence in the new bucket, so a direct same-candidate axis/morphology acceptance repair is not proven yet.
- LLM-classifier ablation `reports/rag_eval/actual_rag_eval_gold29_xlsx_locator_query_anchor_classifier_ablation_v2_nonprod_20260629/report.json` moved gate sufficient/insufficient `12/17 -> 20/9`, but exact/alias and strict E2E stayed `0/29`, and locator candidates shifted to `7` rejections as `missing_validated_required_axes_after_tool`. This supports query-anchor morphology as a target but not as a solved answer-quality lane.
- Repair selection: do not implement the unrestricted local LLM classifier as the repair. Live removed-token evidence includes unsafe structural removals beyond morphology-only intent endings, including examples such as `일산선`, `수인선`, `신논현요양원`, `청운노인요양원`, `2020년`, `2024년`, `2월`, `1월`, and `원달러`. The safe selected repair candidate is a deterministic morphology-only intent-token classifier that may remove generic endings like `명입니까`, `무엇입니까`, `지정된`, `언제입니까`, `얼마인가요`, `변했나요`, and `움직였나요` only when preservation tests prove numeric/date/entity/measure/line/organization anchors remain.
- Closed boundaries: no gate threshold relaxation, no row-specific alias tables, no query/case/row IDs, no filename/workbook/title routing, no formula text or normalized value shortcut, no gold/qrels/labels/expected/supporting evidence routing, no raw XLSX/PDF query-time parsing, no product/production/promotion/fine-tuning/readiness claim.
<!-- agentic_rag_xlsx_v2_query_anchor_rerun_repair_selection_20260629:triage-entry:end -->

<!-- agentic_rag_xlsx_v2_successor_query_anchor_tool_acceptance_diagnostics_20260629:triage-entry:start -->
- agentic_rag_xlsx_v2_successor_query_anchor_tool_acceptance_diagnostics_20260629: Added compact report-only `query_anchor_tool_acceptance_diagnostic` for future XLSX locator reports. The diagnostic is designed to answer whether `missing_query_anchor_after_tool` is blocking otherwise complete source-owned candidates, whether candidates also miss validated axes, or whether no validated-axis evidence exists at all.
- CP02 RunStore evidence remains unchanged: `reports/rag_eval/actual_rag_eval_gold29_xlsx_locator_runstore_v2_generalization_checkpoint_nonprod_20260622/run.sqlite` has `72` candidates, `6` accepted, `66` rejected as `missing_query_anchor_after_tool`, and all candidates are XLSX. CP02 planner fields are blank for source_family_hint/query_task, so the new projection reports those rows as `unknown` rather than inferring a route.
- The likely next safe repair class is query-anchor normalization/router diagnostics for generic Korean question endings and morphology-sensitive anchors such as `명입니까`, `무엇입니까`, `지정된`, and suffix forms like `5호선의`, but only after a fresh report/RunStore run shows whether those anchors reject candidates with complete validated axes. Do not jump directly to gate relaxation or row-specific aliases.
- Guardrails: no item/candidate/source/doc IDs are added to the compact projection; no expected/gold/qrels/labels/file/workbook/title/formula/normalized-value/raw-payload fields are used. The nested diagnostic validator recursively rejects forbidden key surfaces, checks parent/nested count parity, and keeps SQLite RunStore table schema unexpanded under the existing `actual_rag_eval.xlsx_locator_tool_execute_once.v1` schema string. Latest/current/status/source-registry/gold/silver/denominator surfaces stay closed.
<!-- agentic_rag_xlsx_v2_successor_query_anchor_tool_acceptance_diagnostics_20260629:triage-entry:end -->

<!-- agentic_rag_xlsx_v2_checkpoint03_source_native_repairs_20260629:triage-entry:start -->
- agentic_rag_xlsx_v2_checkpoint03_source_native_repairs_20260629: Checkpoint 03 explicitly deferred XLSX source-native code repair because the current evidence does not prove a safe general construction gap. Checkpoint 02 RunStore `reports/rag_eval/actual_rag_eval_gold29_xlsx_locator_runstore_v2_generalization_checkpoint_nonprod_20260622/run.sqlite` had `6` accepted locator candidates and `66` rejected candidates, all rejected with `missing_query_anchor_after_tool`; `complete_validated_axis_candidate_count=0` and `validated_axis_split_across_candidates=0`. That points to query-anchor/tool acceptance diagnostics rather than a safe `display_value`, date-alias, citation-format, or gate-threshold repair.
- Strict quality remains closed: gold `reports/rag_eval/actual_rag_eval_gold29_xlsx_pdf_candidate_surface_v2_generalization_checkpoint_det_nonprod_20260622/report.json` keeps strict E2E `0/29=0.0` and exact/alias answer correctness `0/29=0.0` despite evidence_recall@10 `22/29=0.758621`; silver `reports/rag_eval/actual_rag_eval_silver500_xlsx_candidate_surface_v2_generalization_checkpoint_det_nonprod_20260622/report.json` allows only `8/500` rows and has strict metrics `0/0=None`. Treat this as diagnostic-only measured evidence, not a release or broad quality signal.
- Implemented triage-safe diagnostic: new single-report runs now include `pdf_source_native_decomposition` with page, bbox, page+bbox co-location, section/table axis, OCR confidence, and low-trust OCR counters. The block is `report_only_diagnostic`, `official_metric=false`, `official_metric_input_rows=0`, and uses existing source-owned report context fields only; it does not read raw PDF/XLSX files at query time and does not consume expected/gold/qrels/labels/ids.
- Next safe target remains source-native/query-anchor/tool acceptance decomposition. Do not use per-row alias tables, query IDs, gold/evidence/qrels/label/answerability routing, filename/workbook/title routing, formula text, normalized values as runtime answer shortcuts, gate threshold relaxation, row-specific score targeting, raw workbook/PDF query-time parsing, release language, training language, or broad solved-claim wording.
<!-- agentic_rag_xlsx_v2_checkpoint03_source_native_repairs_20260629:triage-entry:end -->

<!-- xlsx_pdf_source_native_candidate_surface_rebuild_v2_20260621:triage-entry:start -->
- xlsx_pdf_source_native_candidate_surface_rebuild_v2_20260621: Current V2 candidate surface is metric-blocked after a later stopped exact V2 recreate. The stale manifest `reports/rag_eval/weaviate_source_atom_index_manifest_nonprod_route_selected_candidate_surface_v2/index_manifest.json` still has file-byte hash `sha256:f59ff8c4cae77eb15dc75d88291a3452bd1cbac8ba6df2e75e9b9f61728b2151` and old complete-manifest fields, but lacks `collection_recreated_this_run=true`; current code blocks it with `candidate_surface_complete_manifest_collection_recreate_missing`. The stopped checkpoint `reports/rag_eval/weaviate_source_atom_index_manifest_nonprod_route_selected_candidate_surface_v2/index_checkpoint.json` records `completed_count=27712`, `upserted_count_this_run=27712`, `skipped_count_this_run=0`, SHA-256 `9DE1AF9C0E362AA0DA6455EDEF612E918F08C6F2042C13BB83DC4C5EC67C3FF7`, and is dirty/partial checkpoint evidence only. Current readiness also blocks complete-looking stale manifests when the referenced checkpoint is newer than the manifest or count-mismatched.
- Prior V2 local validated-axis artifacts are historical control evidence, not the current checkpoint: gold `reports/rag_eval/actual_rag_eval_gold29_xlsx_pdf_candidate_surface_v2_complete_manifest_llm_validated_axis_det_nonprod_20260621/report.json`, silver `reports/rag_eval/actual_rag_eval_silver500_xlsx_candidate_surface_v2_complete_manifest_llm_validated_axis_det_nonprod_20260621/report.json`, and RunStore `reports/rag_eval/actual_rag_eval_gold29_xlsx_locator_runstore_v2_complete_manifest_llm_validated_axis_nonprod_20260621/run.sqlite`.
- Historical gold source-date-alias diagnostic was mixed and did not move strict answer quality: `reports/rag_eval/actual_rag_eval_gold29_xlsx_pdf_candidate_surface_v2_source_date_alias_llm_validated_axis_det_nonprod_20260621/report.json` kept `official_metric=false`, `official_metric_input_rows=0`, and `bounded_evidence_gate_v1`. Versus exact baseline `reports/rag_eval/actual_rag_eval_gold29_xlsx_pdf_vector_db_det_reindexed_20260615/report.json`, gate allowed/insufficient stayed at the prior control level after `5/24 -> 20/9`, unsupported-after-gate stayed `0.0`, strict E2E stayed `0/29=0.0`, exact/alias answer correctness stayed `0/29=0.0`, and residual `selected_evidence_has_value_missing_axis` remained `5`. Versus prior control, provisional judged/E2E improved by one row (`4/29 -> 5/29`, `3/29 -> 4/29`) and strict evidence_recall@10 rose to `24/29`, but citation-supported fell `41 -> 36`.
- Historical silver source-date-alias remained diagnostic churn: `reports/rag_eval/actual_rag_eval_silver500_xlsx_candidate_surface_v2_source_date_alias_llm_validated_axis_det_nonprod_20260621/report.json` moved gate allowed/insufficient `7/493 -> 44/456` versus baseline and `43/457 -> 44/456` versus control, but the net `+1` hid `17 gate flips` (`9` gained, `8` lost). It kept unsupported-after-gate `0.0`, moved citation-supported rows `379 -> 377` versus baseline, provisional judged correctness was `4/500=0.008`, provisional/inferred E2E stayed `0.0`, and residuals were `selected_evidence_has_value_missing_axis=444`, `no_residual=43`, `not_xlsx_pdf=13`. Strict silver metrics remain N/A.
- Historical RunStore locator triage: `reports/rag_eval/actual_rag_eval_gold29_xlsx_locator_runstore_v2_source_date_alias_llm_validated_axis_nonprod_20260621/run.sqlite` emitted only `run.sqlite`, records `official_metric=0`, `official_metric_input_rows=0`, `anchor_classifier_model=gemma4-e2b-local`, `anchor_classifier_raw_payload_written=0`, 29 items, 148 retrieved contexts, 48 selected-evidence rows, 2 invocations, 7 XLSX candidates, 0 accepted for re-gating, all 7 rejected as `missing_validated_required_axes_after_tool`, and no before/after gate delta. `source_date_aliases` appeared on 2 rejected candidates. This validates storage/observability and alias propagation on rejected candidates but does not validate locator acceptance mechanism.
- Remaining XLSX bottleneck: source-date-alias propagation alone does not keep `period`, `row_entity`, `target_column`, and `display_value` together through candidate acceptance. The next safe XLSX checkpoint should add report/RunStore decomposition by item id, source family, query task, gate flip direction, and residual class, then target source-owned same-candidate package assembly without title/file/workbook, formula, normalized-value, ID, expected, qrels, label, or row-specific shortcuts.
- Remaining PDF bottleneck: 3 PDF residuals remain. Current PDF retrieved contexts preserve page and bbox metadata, yet section/table axes are sparse and there is no query-level `pdf_page_hit`, `bbox_overlap`, or `exact_bbox` decomposition in these artifacts. Keep PDF work diagnostic-only and focused on page/bbox/section/table propagation, not OCR or source intake unless a later approval opens that gate.
- Boundary and next checkpoint: current V2 metric readiness is blocked until a fresh verified complete manifest records exact V2 recreate evidence. Protected hashes for `latest.json`, `latest_text_gold.json`, and `runs.jsonl` remain unchanged, protected namespace diff is empty, checkpoint raw-payload/secrets scans are clean, and external archives were not profiled, indexed, registered, or used. Do not claim official metrics, product success, production readiness, promotion readiness, or plateau break; do not open source intake/indexing, latest/current pointers, gold/silver/qrels/labels/expected/denominator mutation, raw XLSX/PDF query-time parsing, formula/normalized-value shortcuts, title/file shortcuts, row-specific repairs, retrieved-context-only citation promotion, or gate loosening.
<!-- xlsx_pdf_source_native_candidate_surface_rebuild_v2_20260621:triage-entry:end -->
